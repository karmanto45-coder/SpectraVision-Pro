...
def build_separability_figure(result, wavenumber, name_a="Kelompok A", name_b="Kelompok B"):
    ...
    # add_vrect() TIDAK membuat entri legend secara otomatis di Plotly --
    # tanpa ini area kuning/hijau di grafik tetap terlihat tapi legendnya
    # kosong (tidak ada keterangan artinya apa). Tambahkan dua trace
    # dummy (garis lebar, tak terlihat sebagai garis, dipakai HANYA
    # untuk keterangan warna di legend) yang mewakili tiap warna.
    if result["strong_regions"]:
        fig.add_trace(go.Scatter(
            x=[None], y=[None], mode="markers",
            marker=dict(size=12, color="gold", symbol="square"),
            name="Region pembeda KUAT (SNR≥3) / STRONG region",
            showlegend=True,
        ), row=3, col=1)
    if result["candidate_regions"]:
        fig.add_trace(go.Scatter(
            x=[None], y=[None], mode="markers",
            marker=dict(size=12, color="lightgreen", symbol="square"),
            name="Region KANDIDAT (SNR≥1) / CANDIDATE region",
            showlegend=True,
        ), row=3, col=1)
