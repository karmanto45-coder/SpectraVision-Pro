...
def abs_area(y, x):
    """
    Luas trapezoid |y| terhadap x, SELALU non-negatif.

    PERBAIKAN: np.trapezoid(y, x) bernilai NEGATIF kalau x (wavenumber)
    menurun (4000 -> 400), yang merupakan urutan standar banyak file
    FTIR. Kode lama memakai `if area > 0:` sehingga untuk file
    wavenumber-menurun normalisasi area DILEWATI DIAM-DIAM padahal
    checkbox "Normalisasi" aktif dan log mengklaim normalisasi sudah
    diterapkan. Luas fisik selalu positif -> pakai nilai mutlak.
    """
    y = np.asarray(y, dtype=float)
    x = np.asarray(x, dtype=float)
    val = (np.trapezoid(y, x) if hasattr(np, "trapezoid") else np.trapz(y, x))
    return abs(float(val))


def preprocess(spectra_matrix, wavenumber,
               do_norm=True, do_smooth=False, do_baseline=False):
    proc = np.array(spectra_matrix, dtype=float, copy=True)
    if not np.all(np.isfinite(proc)):
        raise ValueError(
            "Matriks spektra mengandung NaN/Inf — bersihkan data sebelum "
            "pra-pemrosesan / Spectra matrix contains NaN/Inf.")
    if do_smooth:
        n_pts = proc.shape[0]
        win = min(11, n_pts if n_pts % 2 == 1 else n_pts - 1)
        if win < 5:
            raise ValueError(
                f"Smoothing SG butuh minimal 5 titik data (hanya {n_pts}) / "
                f"Savitzky-Golay smoothing needs at least 5 data points "
                f"(only {n_pts}).")
        poly = min(3, win - 2)
        for i in range(proc.shape[1]):
            proc[:, i] = savgol_filter(proc[:, i], win, poly)
    if do_baseline:
        for i in range(proc.shape[1]):
            proc[:, i] -= proc[:, i].min()
    if do_norm:
        for i in range(proc.shape[1]):
            area = abs_area(proc[:, i], wavenumber)
            if area > 0:
                proc[:, i] /= area
    return proc
...
def postprocess_mcr_spectra(S, wavenumber,
    ...
    if do_smooth:
        n_pts = S_proc.shape[1]
        win  = max(5, sg_window if sg_window % 2 == 1 else sg_window + 1)
        poly = min(sg_poly, win - 2)
        # PERBAIKAN: jangan diam-diam gagal. Window tidak boleh melebihi
        # jumlah titik, dan log HANYA boleh mengklaim smoothing kalau
        # benar-benar diterapkan (dulu `except: pass` + log tetap
        # mengklaim "Smoothing" walau spektra tidak berubah sama sekali).
        if win > n_pts:
        ...
        if win >= 5 and poly >= 1:
            for i in range(S_proc.shape[0]):
                try:
                    ...
                    pass
        if n_ok == S_proc.shape[0]:
            log.append(f"Smoothing: Savitzky-Golay (window={win}, poly={poly})")
        elif n_ok == 0:
            log.append("Smoothing: DILEWATI / SKIPPED (parameter SG tidak valid "
                       "untuk jumlah titik data ini)")
        else:
            log.append(f"Smoothing: Savitzky-Golay (window={win}, poly={poly}) "
                       f"HANYA {n_ok}/{S_proc.shape[0]} komponen")
    ...
    if do_norm:
        ...
        for i in range(S_proc.shape[0]):
            ...
            n_norm_ok += int(done)
        if n_norm_ok == S_proc.shape[0]:
            log.append(f"Normalization: {norm_method}")
        else:
            log.append(f"Normalization: {norm_method} — HANYA {n_norm_ok}/"
                       f"{S_proc.shape[0]} komponen (sisanya bernilai nol/datar, dilewati)")
...
def _init_mcr(D, n_components, init_method="pca", S_init=None, s_nonneg=True):
    ...
    if S_init is not None:
        ...
        if S0.ndim != 2 or S0.shape[0] != n_components or S0.shape[1] != n:
            raise ValueError(
                f"S_init harus berbentuk ({n_components} x {n}) — diterima "
                f"{S0.shape}. Jumlah spektra initial guess harus sama dengan "
                f"jumlah komponen dan sudah di-resample ke grid data.")
        if s_nonneg:
            S0 = np.maximum(S0, 0)
...
def run_mcr_als(D, n_components, max_iter=200, tol=1e-6,
    ...
    D = np.array(D, dtype=float)
    if D.ndim != 2:
        raise ValueError("D harus matriks 2D (n_sampel x n_wavenumber).")
    m, n = D.shape
    if not np.all(np.isfinite(D)):
        raise ValueError(
            "Data mengandung NaN/Inf — MCR-ALS tidak bisa dijalankan / "
            "Data contains NaN/Inf — MCR-ALS cannot run.")
    n_components = int(n_components)
    if n_components < 1 or n_components > min(m, n):
        raise ValueError(
            f"Jumlah komponen ({n_components}) tidak valid untuk data "
            f"{m} sampel x {n} titik — harus 1..{min(m, n)} / "
            f"Number of components ({n_components}) invalid for {m} samples "
            f"x {n} points — must be 1..{min(m, n)}.")
...
def compare_two_spectra(wn_a, sp_a, wn_b, sp_b, window_mode="full",
                        ...
                        sg_window=11, sg_poly=3, shift_tolerance=5):
    """
    ...
    deriv = round(derivative_corr(sp_a_c, sp_b_c, order=deriv_order,
                                  window=sg_window, poly=sg_poly), 4)
...
def batch_match(query_spec, query_wn, library_entries,
                ...
                deriv_order=1, sg_window=11, sg_poly=3, shift_tolerance=5):
    """
    Parameters tambahan
    --------------------
    ...
    sg_window : lebar window Savitzky-Golay untuk derivative_corr()
              (harus ganjil dan > sg_poly + 1). Default 11.
    sg_poly : orde polinomial Savitzky-Golay untuk derivative_corr().
              Default 3.
    ...
    for entry in library_entries:
        ...
        deriv = round(derivative_corr(sp_q_c, sp_r_c, order=deriv_order,
                                      window=sg_window, poly=sg_poly), 4)
...
# ════════════════════════════════════════════════════════════════
# ROTATIONAL AMBIGUITY — MCR-BANDS & RMSERA  [BARU]
# ════════════════════════════════════════════════════════════════
#
# Latar belakang: solusi MCR-ALS pada dataset first-order (satu set
# spektrum campuran tanpa dimensi orde-kedua/hyphenated, seperti kasus
# ATR-FTIR di SpectraVision Pro) pada umumnya TIDAK unik — banyak
# pasangan (C, S) berbeda yang menghasilkan rekonstruksi D_hat = C @ S
# yang identik ("rotational ambiguity"), selama pasangan itu tetap
# memenuhi constraint yang sama (non-negativity, closure, dst).
# Scorecard yang sudah ada (LOF, RMSE, NNV, EV, SRI) mengukur SEBERAPA
# BAIK satu solusi cocok dengan data dan konsisten di berbagai k —
# tapi TIDAK mengukur SEBERAPA LEBAR rentang solusi lain yang sama-
# sama valid. Dua fungsi di bawah mengisi celah itu.
#
# compute_mcr_bands()
#   Mengestimasi rentang (band) nilai C dan S yang feasible, dengan
#   menjelajahi ruang rotasi C → C@T, S → T⁻¹@S (T persegi
#   n_komponen × n_komponen). Rotasi semacam ini TIDAK PERNAH mengubah
#   rekonstruksi sama sekali, karena C@T@T⁻¹@S = C@S persis secara
#   aljabar — HANYA diterima ("feasible") jika hasil rotasinya tetap
#   memenuhi constraint yang sedang dipakai (non-negativity C/S,
#   closure).
#
#   PENTING — keterbatasan metode: ini adalah pendekatan Monte Carlo /
#   random-sampling terhadap ruang rotasi, BUKAN re-implementasi
#   persis algoritma MCR-BANDS asli (Jaumot & Tauler, 2010, Chemom.
#   Intell. Lab. Syst. 103, 96-107), yang mencari batas ekstrem
#   feasible per variabel melalui optimasi terkendala (NLP) eksak.
#   Sampling acak jauh lebih murah secara komputasi dan cukup memadai
#   untuk n_komponen kecil (2-6, kasus umum di sini), tapi bisa
#   MELEWATKAN sebagian ruang feasible — band yang dilaporkan adalah
#   estimasi batas bawah dari lebar band sebenarnya (cenderung under-
#   estimate ambiguitas, bukan over-estimate). Naikkan n_samples jika
#   acceptance_rate rendah atau band tampak sempit tak wajar.
#
# compute_rmsera()
#   Menghitung parameter RMSERA sesuai definisi Alcaraz, Vera-Candioti,
#   Culzoni, Goicoechea, et al., Anal. Chem. 2020, 92(10), 7255-7263,
#   "A New Parameter for Measuring the Prediction Uncertainty Produced
#   by Rotational Ambiguity...". Rumus asli: RMSERA berada pada
#   rentang [δRA/√12, δRA/√3], dengan δRA = selisih maksimum-minimum
#   nilai konsentrasi analit yang diprediksi dari profil konsentrasi
#   MCR yang berada dalam rentang solusi feasible. Batas bawah
#   (δRA/√12) mengasumsikan nilai sebenarnya berada di TENGAH band
#   feasible (kasus terbaik); batas atas (δRA/√3) mengasumsikan nilai
#   sebenarnya berada di salah satu UJUNG band (kasus terburuk).
#
#   ADAPTASI untuk SpectraVision Pro: publikasi asli menurunkan δRA
#   dari konteks kalibrasi orde-kedua (luas puncak profil konsentrasi
#   MCR dibandingkan kalibrasi eksternal). Metode RMSERA/N-BANDS juga
#   sudah dilaporkan diterapkan pada data first-order (NIR/UV-Vis/
#   fluoresensi) tanpa kalibrasi orde-kedua — sesuai konteks tersebut,
#   di sini δRA diambil langsung dari lebar band KONSENTRASI RELATIF
#   hasil compute_mcr_bands() (skala C hasil MCR-ALS, BUKAN satuan
#   konsentrasi absolut seperti mg/L). Untuk konversi ke satuan
#   absolut, kalikan hasilnya dengan faktor kalibrasi eksternal jika
#   tersedia (mis. dari kurva kalibrasi standar).

def compute_mcr_bands(C, S, s_nonneg=True, c_nonneg=True,
                       closure=False, closure_tol=0.02,
                       fix_intensity_scale=True,
                       n_samples=3000, max_perturbation=0.8,
                       n_scales=8, random_state=None):
    """
    Estimasi band solusi feasible MCR (pendekatan Monte Carlo ruang
    rotasi). Lihat catatan modul di atas untuk keterbatasan metode.

    fix_intensity_scale (PENTING) — MCR-BANDS asli (dan literatur
    ambiguitas MCR pada umumnya) membedakan dua jenis ambiguitas yang
    TERPISAH: (1) intensity/scale ambiguity — skala absolut tiap
    komponen pada dasarnya TIDAK BISA ditentukan dari data saja kecuali
    ada penjangkar skala eksternal (closure yang benar-benar berlaku,
    standar/library acuan, dsb) — ini SELALU sangat lebar tanpa
    penjangkar, dan bukan sesuatu yang "diperbaiki" analisis rotasi;
    dan (2) rotation/shape ambiguity — apakah BENTUK spektra komponen
    bisa dibedakan satu sama lain, terlepas dari skala absolutnya. Untuk
    pertanyaan khas SpectraVision Pro ("apakah komponen A dan B bisa
    dipisahkan?", mis. kasus jahe merah vs jahe emprit yang miripbentuk),
    yang relevan adalah (2), bukan (1). Karena itu default
    fix_intensity_scale=True: setiap kandidat rotasi yang lolos
    constraint dikembalikan skalanya (dikalikan faktor per-komponen)
    agar norma S tiap komponen sama dengan norma S solusi asli, sebelum
    ikut membentuk band — sehingga band yang dilaporkan mencerminkan
    ambiguitas BENTUK, bukan tercampur/didominasi ambiguitas skala.
    Set False untuk melihat ambiguitas total termasuk skala (akan jauh
    lebih lebar, terutama jika closure=False).

    Parameters
    ----------
    C, S            : solusi MCR-ALS yang sudah konvergen.
                      C (n_sampel x n_komponen), S (n_komponen x n_variabel).
    s_nonneg        : apakah non-negativity pada S harus dipertahankan
                      feasible — SAMAKAN dengan pengaturan run MCR-ALS
                      aslinya (False untuk domain spektra turunan).
    c_nonneg        : non-negativity pada C (default True — konsentrasi
                      fisik tidak pernah negatif di domain apapun).
    closure         : apakah closure constraint (jumlah tiap baris C = 1)
                      harus dipertahankan feasible — samakan dengan
                      pengaturan run MCR-ALS aslinya. CATATAN: jika
                      fix_intensity_scale=True, closure dicek pada C
                      setelah penyesuaian skala (konvensi skala
                      "norma-S-tetap"), bukan pada skala mentah hasil
                      rotasi — hasilnya tetap konsisten karena pada
                      T=identitas keduanya sama dengan solusi asli.
    closure_tol     : toleransi absolut penyimpangan closure yang masih
                      dianggap feasible.
    n_samples       : jumlah total percobaan rotasi acak (BUKAN jumlah
                      yang diterima — acceptance_rate bisa jauh <100%,
                      terutama pada dataset dengan constraint ketat).
    max_perturbation: skala perturbasi rotasi terbesar yang dijelajahi,
                      relatif terhadap matriks identitas. Skala
                      dijelajahi bertahap (log-spaced, kecil→besar) agar
                      mencakup band sempit maupun lebar sekaligus.
    n_scales        : jumlah tingkatan skala perturbasi yang dijelajahi.
    random_state    : seed RNG untuk reproducibility hasil.

    Returns
    -------
    dict:
      C_lower, C_upper : (n_sampel x n_komponen) batas bawah/atas C feasible
      S_lower, S_upper : (n_komponen x n_variabel) batas bawah/atas S feasible
      n_tried, n_accepted, acceptance_rate
      fix_intensity_scale : nilai parameter yang dipakai (untuk audit trail)
      recon_diff_max   : deviasi rekonstruksi maksimum di antara rotasi
                         yang diterima vs solusi asli (sanity check —
                         harus mendekati 0, karena rotasi valid secara
                         aljabar tidak mengubah D_hat sama sekali)
    """
    ...
    scale_c = max(1.0, float(np.max(np.abs(C))))
    scale_s = max(1.0, float(np.max(np.abs(S))))
    tol_c = 1e-6 * scale_c
    tol_s = 1e-6 * scale_s
    D_orig = C @ S
    recon_tol = 1e-6 * max(1.0, float(np.max(np.abs(D_orig))))
    norm_orig = np.sqrt(np.sum(S ** 2, axis=1))  # ||S[i]|| per komponen
    ...
    def _try_accept(T):
        nonlocal n_tried, recon_diff_max
        n_tried += 1
        ...
        # ── Konvensi skala: kembalikan norma tiap komponen S ke norma
        # solusi asli (menghilangkan derajat kebebasan skala trivial
        # dari ambiguitas yang dilaporkan). Pada T=identitas ini adalah
        # no-op persis (scale_i=1 untuk semua i).
        if fix_intensity_scale:
            norm_rot = np.sqrt(np.sum(S_rot ** 2, axis=1))
            if np.any(norm_rot < 1e-12):
                return
            scale_i = norm_orig / norm_rot
            S_rot = S_rot * scale_i[:, None]
            C_rot = C_rot / scale_i[None, :]

        if c_nonneg and C_rot.min() < -tol_c:
            return
        if s_nonneg and S_rot.min() < -tol_s:
            return
        if closure:
            row_sums = C_rot.sum(axis=1)
            if np.any(np.abs(row_sums - 1.0) > closure_tol):
                return

        # Sanity check aljabar: rotasi (+ penyesuaian skala per-
        # komponen, yang murni diagonal sehingga tetap invarian) TIDAK
        # BOLEH mengubah rekonstruksi D_hat sama sekali. Jika berbeda
        # signifikan, ada masalah numerik (T mendekati singular) — tolak.
        diff = float(np.max(np.abs((C_rot @ S_rot) - D_orig)))
        if diff > recon_tol:
            return
        recon_diff_max = max(recon_diff_max, diff)

        C_all.append(C_rot)
        S_all.append(S_rot)

    n_scales_eff = max(1, int(n_scales))
    scales = np.geomspace(0.02, max(float(max_perturbation), 0.02), num=n_scales_eff)
    per_scale = max(1, int(n_samples) // n_scales_eff)

    for scale in scales:
        for _ in range(per_scale):
            T = np.eye(n_comp) + rng.normal(scale=scale, size=(n_comp, n_comp))
            _try_accept(T)

    C_stack = np.array(C_all)
    S_stack = np.array(S_all)
    n_accepted = C_stack.shape[0]

    return {
        "C_lower": C_stack.min(axis=0), "C_upper": C_stack.max(axis=0),
        "S_lower": S_stack.min(axis=0), "S_upper": S_stack.max(axis=0),
        "n_tried": n_tried, "n_accepted": n_accepted,
        "acceptance_rate": float(n_accepted / n_tried) if n_tried else 0.0,
        "recon_diff_max": recon_diff_max,
    }


def compute_rmsera(C, C_lower, C_upper):
    """
    Hitung parameter RMSERA (Alcaraz et al., Anal. Chem. 2020) dari
    band konsentrasi feasible hasil compute_mcr_bands(). Lihat catatan
    modul di atas untuk definisi dan adaptasi yang dipakai.

    Parameters
    ----------
    C        : solusi konsentrasi MCR-ALS asli (n_sampel x n_komponen)
    C_lower, C_upper : dari compute_mcr_bands()

    Returns
    -------
    dict:
      delta_ra                 : (n_sampel x n_komponen) = C_upper - C_lower
      rmsera_lower_per_sample  : delta_ra / √12  (kasus terbaik, per sampel)
      rmsera_upper_per_sample  : delta_ra / √3   (kasus terburuk, per sampel)
      summary                  : list of dict per komponen —
                                  delta_ra_rms, delta_ra_max,
                                  rmsera_lower, rmsera_upper (RMS lintas
                                  sampel), rmsera_upper_pct (relatif thd
                                  RMS konsentrasi komponen tsb, %)
    """
    C = np.asarray(C, dtype=float)
    delta_ra = np.maximum(np.asarray(C_upper) - np.asarray(C_lower), 0.0)
    rmsera_lower_ps = delta_ra / np.sqrt(12)
    rmsera_upper_ps = delta_ra / np.sqrt(3)
    ...
    for i in range(n_comp):
        ...
        c_rms = float(np.sqrt(np.mean(C[:, i] ** 2)))
        rel_pct = (100.0 * rms_d / c_rms) if c_rms > 0 else float("nan")
        summary.append({
            "component":        i,
            "delta_ra_rms":      rms_d,
            "delta_ra_max":      float(np.max(d)),
            "rmsera_lower":      rms_d / np.sqrt(12),
            "rmsera_upper":      rms_d / np.sqrt(3),
            "rmsera_upper_pct":  rel_pct,
        })

    return {
        "delta_ra": delta_ra,
        "rmsera_lower_per_sample": rmsera_lower_ps,
        "rmsera_upper_per_sample": rmsera_upper_ps,
        "summary": summary,
    }


def classify_ambiguity_severity(rmsera_upper_pct):
    """
    Klasifikasi kualitatif cepat tingkat ambiguitas rotasi berdasarkan
    RMSERA batas atas relatif (%). CATATAN: ambang batas berikut adalah
    heuristik praktis internal SpectraVision Pro (bukan dari literatur
    RMSERA) — dimaksudkan sebagai panduan interpretasi awal, bukan
    kriteria definitif/tervalidasi.

    < 5%   → rendah  : solusi relatif well-determined
    5-15%  → sedang  : ambiguitas cukup berarti, interpretasi kuantitatif
                        perlu hati-hati
    >= 15% → tinggi  : rentang solusi feasible lebar, hasil kuantitatif
                        sebaiknya tidak ditafsir tanpa constraint tambahan
                        atau informasi independen (mis. standar/library)

    Returns (level, icon, message)
    """
    if rmsera_upper_pct != rmsera_upper_pct:  # NaN check
        return "unknown", "❔", ("Tidak dapat dihitung (RMS konsentrasi komponen = 0)")
    if rmsera_upper_pct < 5:
        return "low", "🟢", "Rendah — solusi relatif well-determined"
    if rmsera_upper_pct < 15:
        return "moderate", "🟡", "Sedang — interpretasi kuantitatif perlu hati-hati"
    return "high", "🔴", "Tinggi — rentang solusi feasible lebar"
