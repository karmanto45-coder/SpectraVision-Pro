...
def find_significant_extrema(spectrum, wavenumber, order=1, prominence_frac=0.1):
    ...
    if order % 2 == 1:
        ...
        return sorted(float(wavenumber[i]) for i in valley_idx)


def peak_position_match(query_spec, query_wn, ref_spec, ref_wn,
                        order=1, tolerance_cm=6.0, prominence_frac=0.1):
    """
    Cocokkan posisi pita (band center) antara spektrum turunan query
    (mis. komponen hasil MCR) dan spektrum turunan referensi murni —
    ini metrik identifikasi yang independen dari amplitudo/skala
    turunan, karena hanya membandingkan POSISI wavenumber, bukan bentuk
    kurva. Direkomendasikan sebagai pelengkap wajib untuk cosine/pearson/
    derivative_corr saat interpretasi visual sulit dilakukan (kasus
    umum pada 2nd derivative dengan banyak lembah kecil yang mirip).
    ...
    """
    q_pos = find_significant_extrema(query_spec, query_wn, order, prominence_frac)
    r_pos = find_significant_extrema(ref_spec, ref_wn, order, prominence_frac)

    matched_pairs = []
    used_ref = set()
    unmatched_query = []
    for qp in q_pos:
        best_j, best_d = -1, tolerance_cm + 1e-9
        for j, rp in enumerate(r_pos):
            if j in used_ref:
                continue
            d = abs(qp - rp)
            if d < best_d:
        ...
        if best_j >= 0:
            used_ref.add(best_j)
            matched_pairs.append((qp, r_pos[best_j], qp - r_pos[best_j]))
        else:
            unmatched_query.append(qp)
...
def batch_match_derivative(query_deriv_spec, query_wn, deriv_library_entries,
    ...
    Perbedaan dari batch_match() biasa:
      ...
      3. Menambahkan peak_position_match() sebagai metrik utama —
         mencocokkan POSISI band (lembah untuk 2nd derivative, zero-
         crossing untuk 1st derivative), bukan cuma bentuk kurva,
         karena interpretasi visual bentuk turunan kedua sangat sulit
         dan gampang salah tafsir.

    deriv_library_entries : list of dict, tiap entri wajib punya key
        "deriv_order" selain "id","name","category","wavenumber","spectrum".

    Returns : list of dict, sudah diurutkan (composite = rata-rata
              pearson + derivative_corr + shift_tolerant + match_fraction
              posisi pita), top_n entri, dengan flag "ambiguous" seperti
              batch_match() biasa.
    """
    if grid_interval != "auto":
        try:
    ...
    for entry in deriv_library_entries:
        ...
        pear = round(pearson_corr(sp_q_c, sp_r_c), 4)
        deriv_corr_score = round(derivative_corr(sp_q_c, sp_r_c, order=1), 4)
        shift_score, shift_lag = shift_tolerant_corr(sp_q_c, sp_r_c, max_shift=5)
        shift_score = round(shift_score, 4)
        ...
        composite = round(
            (pear + deriv_corr_score + shift_score + peak_info["match_fraction"]) / 4, 4
        )
        ...
        results.append({
            ...
            "pearson": pear, "derivative": deriv_corr_score,
            "shift": shift_score, "shift_lag": shift_lag,
