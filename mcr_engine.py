...
def _init_mcr(D, n_components, S_init=None):
    """
    Inisialisasi C dan S untuk MCR-ALS.

    Jika S_init disediakan (spektra eksternal dari library):
      → gunakan sebagai S awal, hitung C dari S
    Jika tidak:
      → gunakan PCA scores sebagai C awal (shift-to-positive)
      → lebih baik dari np.abs() karena mempertahankan struktur variansi

    Returns: C_init (m × k), S_init_out (k × n)
    """
    ...
    if S_init is not None:
        ...
        # PCA-based initialization dengan shift-to-positive
        pca = PCA(n_components=n_components)
        C_raw = pca.fit_transform(D)
        # Shift ke positif per kolom (pertahankan struktur variansi)
...
def run_mcr_als(D, n_components, max_iter=200, tol=1e-6,
    ...
    # ── Inisialisasi ──────────────────────────────────────────
    C, S0 = _init_mcr(D, n_components, S_init)
    if S0 is not None:
        S = S0.copy()
