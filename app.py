...
from mcr_engine import (preprocess, detect_components, run_mcr_als,
                        ...
                        compare_two_spectra,
                        compute_mcr_bands, compute_rmsera,
...
tab_rep     = tabs[8] if is_admin() else tabs[6]
...
with tab_mcr:
    if "spectra" not in st.session_state:
        ...
        if st.button(f"▶  {t('Jalankan MCR-ALS','Run MCR-ALS')}",
            ...
            with st.spinner(t("Menjalankan MCR-ALS...","Running MCR-ALS...")):
                ...
                for _key in ["mcr_S_proc","mcr_proc_log","match_results",
                             ...
                             "mcr_bands_result","mcr_rmsera_result"]:
        ...
        if "mcr_S" in st.session_state:
            ...
            # ── Ambiguitas Rotasi — RMSERA & MCR-BANDS [BARU] ──────
            with st.expander(
                t("🎯 Ambiguitas Rotasi — RMSERA & MCR-BANDS (opsional)",
                  "🎯 Rotational Ambiguity — RMSERA & MCR-BANDS (optional)"),
                expanded=False
            ):
                st.caption(t(
                    "Scorecard di atas mengukur seberapa BAIK solusi ini cocok dengan "
                    "data. Modul ini mengukur hal yang berbeda: seberapa LEBAR rentang "
                    "solusi (C, S) lain yang **sama-sama valid** (fit identik, memenuhi "
                    "constraint yang sama) — fenomena *rotational ambiguity* pada MCR. "
                    "Metode: sampling Monte Carlo pada ruang rotasi C→C·T, S→T⁻¹·S "
                    "(bukan re-implementasi eksak algoritma MCR-BANDS asli/NLP dari "
                    "Jaumot & Tauler 2010 — lihat catatan lengkap di kode `mcr_engine.py`). "
                    "RMSERA mengikuti definisi Alcaraz et al., *Anal. Chem.* 2020, "
                    "92(10), 7255-7263, diadaptasi ke skala konsentrasi relatif MCR-ALS "
                    "(bukan satuan absolut mg/L, karena tidak ada kalibrasi eksternal).",
                    "The scorecard above measures how WELL this solution fits the data. "
                    "This module measures something different: how WIDE the range of "
                    "other, equally-valid (C, S) solutions is — the *rotational "
                    "ambiguity* phenomenon in MCR. Method: Monte Carlo sampling over the "
                    "rotation space C→C·T, S→T⁻¹·S (not an exact re-implementation of "
                    "the original MCR-BANDS NLP algorithm from Jaumot & Tauler 2010 — see "
                    "full notes in `mcr_engine.py`). RMSERA follows the definition from "
                    "Alcaraz et al., *Anal. Chem.* 2020, 92(10), 7255-7263, adapted to "
                    "the relative MCR-ALS concentration scale (not absolute mg/L units, "
                    "since there is no external calibration)."
                ))

                rb1, rb2, rb3 = st.columns(3)
                n_samples_bands = rb1.number_input(
                    t("Jumlah sampling rotasi","Rotation samples"),
                    min_value=500, max_value=50000, value=6000, step=500,
                    key="mcr_bands_nsamples",
                    help=t("Lebih besar = estimasi band lebih stabil, tapi lebih lambat.",
                           "Larger = more stable band estimate, but slower.")
                )
                fix_intensity = rb2.checkbox(
                    t("Isolasi ambiguitas bentuk (fix skala)","Isolate shape ambiguity (fix scale)"),
                    value=True, key="mcr_bands_fix_intensity",
                    help=t(
                        "Direkomendasikan ON. Tanpa closure, skala absolut tiap "
                        "komponen memang secara matematis tidak bisa ditentukan dari "
                        "data saja (intensity ambiguity) — ini SELALU sangat lebar dan "
                        "bukan sesuatu yang bisa 'diperbaiki' analisis rotasi. Opsi ini "
                        "menghilangkan derajat kebebasan skala tersebut dari band yang "
                        "dilaporkan, sehingga angka mencerminkan apakah BENTUK spektra "
                        "komponen bisa dibedakan satu sama lain — pertanyaan yang lebih "
                        "relevan untuk kasus seperti jahe merah vs jahe emprit. Matikan "
                        "untuk melihat ambiguitas total termasuk skala (akan tampak jauh "
                        "lebih lebar, terutama jika closure tidak dipakai).",
                        "Recommended ON. Without closure, each component's absolute "
                        "scale genuinely cannot be determined from the data alone "
                        "(intensity ambiguity) — this is ALWAYS very wide and is not "
                        "something rotation analysis can 'fix'. This option removes "
                        "that scale degree of freedom from the reported bands, so the "
                        "number reflects whether component SHAPES can be told apart — "
                        "more relevant for cases like red vs white ginger. Turn off to "
                        "see total ambiguity including scale (will look far wider, "
                        "especially without closure)."
                    )
                )
                run_bands = rb3.button(
                    f"▶ {t('Hitung Band & RMSERA','Compute Bands & RMSERA')}",
                    use_container_width=True
                )

                if run_bands:
                    cu_bands = st.session_state.get("mcr_diag", {}).get("constraints_used", {})
                    with st.spinner(t("Menjelajahi ruang rotasi...","Exploring rotation space...")):
                        bands_result = compute_mcr_bands(
                            C_res, S_res,
                            s_nonneg=cu_bands.get("s_nonneg", True),
                            closure=cu_bands.get("closure", False),
                            fix_intensity_scale=fix_intensity,
                            n_samples=int(n_samples_bands),
                            random_state=42,
                        )
                        rmsera_result = compute_rmsera(
                            C_res, bands_result["C_lower"], bands_result["C_upper"]
                        )
                    st.session_state["mcr_bands_result"]  = bands_result
                    st.session_state["mcr_rmsera_result"] = rmsera_result

                if "mcr_rmsera_result" in st.session_state:
                    bands_result  = st.session_state["mcr_bands_result"]
                    rmsera_result = st.session_state["mcr_rmsera_result"]

                    n_acc = bands_result["n_accepted"]
                    n_try = bands_result["n_tried"]
                    st.caption(t(
                        f"Rotasi feasible diterima: {n_acc:,} / {n_try:,} "
                        f"(acceptance rate {bands_result['acceptance_rate']*100:.2f}%) · "
                        f"cek konsistensi rekonstruksi: {bands_result['recon_diff_max']:.2e} "
                        f"(harus ≈ 0)",
                        f"Feasible rotations accepted: {n_acc:,} / {n_try:,} "
                        f"(acceptance rate {bands_result['acceptance_rate']*100:.2f}%) · "
                        f"reconstruction consistency check: {bands_result['recon_diff_max']:.2e} "
                        f"(should be ≈ 0)"
                    ))
                    if n_acc < 30:
                        st.warning(t(
                            "⚠️ Rotasi feasible yang diterima sangat sedikit (<30). Band "
                            "yang dilaporkan kemungkinan UNDER-ESTIMATE ambiguitas "
                            "sebenarnya (bukan bukti solid bahwa ambiguitas benar-benar "
                            "nol) — sampling belum cukup menjelajahi ruang feasible. "
                            "Naikkan 'Jumlah sampling rotasi' dan hitung ulang sebelum "
                            "menyimpulkan solusi ini well-determined.",
                            "⚠️ Very few feasible rotations were accepted (<30). The "
                            "reported bands are likely an UNDER-ESTIMATE of the true "
                            "ambiguity (not solid evidence that ambiguity is truly zero) "
                            "— sampling hasn't sufficiently explored the feasible space "
                            "yet. Increase 'Rotation samples' and recompute before "
                            "concluding this solution is well-determined."
                        ))

                    # ── Tabel ringkasan RMSERA per komponen ────────
                    rows_rmsera = []
                    for s in rmsera_result["summary"]:
                        lvl, icon, msg = classify_ambiguity_severity(s["rmsera_upper_pct"])
                        rel_str = (f"{s['rmsera_upper_pct']:.2f}%"
                                   if s["rmsera_upper_pct"] == s["rmsera_upper_pct"]
                                   else "—")
                        rows_rmsera.append({
                            t("Komponen","Component"): f"{s['component']+1}",
                            "RMSERA (lower, δRA/√12)": f"{s['rmsera_lower']:.5f}",
                            "RMSERA (upper, δRA/√3)":  f"{s['rmsera_upper']:.5f}",
                            t("RMSERA upper (relatif %)","RMSERA upper (relative %)"): rel_str,
                            t("Status","Status"): f"{icon} {msg}",
                        })
                    st.dataframe(pd.DataFrame(rows_rmsera), use_container_width=True,
                                 hide_index=True)

                    # ── Grafik band spektra (MCR-BANDS pada S) ─────
                    st.markdown(f'<p class="sec-hdr" style="margin-top:0.8rem;">'
                                f'{t("Band spektra murni (MCR-BANDS)","Pure spectra bands (MCR-BANDS)")}'
                                f'</p>', unsafe_allow_html=True)
                    comp_labels_band = [f"{t('Komponen','Component')} {i+1}" for i in range(nc)]
                    comp_pick = st.selectbox(
                        t("Pilih komponen untuk ditampilkan","Select component to display"),
                        comp_labels_band, key="mcr_bands_comp_pick"
                    )
                    ci_band = comp_labels_band.index(comp_pick)

                    fig_band = go.Figure()
                    fig_band.add_trace(go.Scatter(
                        x=wn, y=bands_result["S_upper"][ci_band],
                        line=dict(width=0), showlegend=False, hoverinfo="skip"
                    ))
                    fig_band.add_trace(go.Scatter(
                        x=wn, y=bands_result["S_lower"][ci_band],
                        fill="tonexty", fillcolor="rgba(125,211,252,0.22)",
                        line=dict(width=0),
                        name=t("Rentang feasible","Feasible range")
                    ))
                    fig_band.add_trace(go.Scatter(
                        x=wn, y=S_res[ci_band], mode="lines",
                        line=dict(color="#f97316", width=1.8),
                        name=t("Solusi MCR-ALS","MCR-ALS solution")
                    ))
                    ...
                    st.caption(t(
                        "Pita biru menunjukkan rentang bentuk spektra yang masih "
                        "sama-sama valid (fit identik terhadap data, memenuhi "
                        "constraint yang sama) selain solusi oranye yang ditampilkan "
                        "di tab ini. Pita sempit → bentuk komponen ini relatif "
                        "well-determined; pita lebar → berhati-hatilah menafsirkan "
                        "bentuk puncak komponen ini secara kuantitatif tanpa info "
                        "tambahan (constraint lain, library acuan, dsb).",
                        "The blue band shows the range of component shapes that are "
                        "equally valid (identical fit to the data, same constraints "
                        "satisfied) alongside the orange solution shown elsewhere in "
                        "this tab. Narrow band → this component's shape is relatively "
                        "well-determined; wide band → be cautious interpreting this "
                        "component's peak shape quantitatively without extra "
                        "information (other constraints, reference library, etc.)."
                    ))
...
with tab_rep:
    if "mcr_S" not in st.session_state:
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            # RMSERA / MCR-BANDS (opsional — hanya jika sudah dihitung
            # di tab Analisis MCR) [BARU]
            if "mcr_rmsera_result" in st.session_state:
                rmsera_exp = st.session_state["mcr_rmsera_result"]
                bands_exp  = st.session_state["mcr_bands_result"]
                rows_exp = []
                for s in rmsera_exp["summary"]:
                    lvl_exp, icon_exp, msg_exp = classify_ambiguity_severity(s["rmsera_upper_pct"])
                    rows_exp.append({
                        t("Komponen","Component"): s["component"] + 1,
                        "delta_RA (RMS)": s["delta_ra_rms"],
                        "delta_RA (max)": s["delta_ra_max"],
                        "RMSERA_lower (δRA/√12)": s["rmsera_lower"],
                        "RMSERA_upper (δRA/√3)": s["rmsera_upper"],
                        t("RMSERA upper relatif (%)","RMSERA upper relative (%)"): s["rmsera_upper_pct"],
                        t("Status ambiguitas","Ambiguity status"): f"{icon_exp} {msg_exp}",
                    })
                rows_exp.append({
                    t("Komponen","Component"): t("— Info sampling —","— Sampling info —"),
                    "delta_RA (RMS)": None, "delta_RA (max)": None,
                    "RMSERA_lower (δRA/√12)": None, "RMSERA_upper (δRA/√3)": None,
                    t("RMSERA upper relatif (%)","RMSERA upper relative (%)"):
                        f"n_accepted={bands_exp['n_accepted']}/{bands_exp['n_tried']} "
                        f"({bands_exp['acceptance_rate']*100:.2f}%)",
                    t("Status ambiguitas","Ambiguity status"):
                        t("Metode: Monte Carlo rotation sampling (bukan NLP MCR-BANDS eksak)",
                          "Method: Monte Carlo rotation sampling (not exact NLP MCR-BANDS)"),
                })
                pd.DataFrame(rows_exp).to_excel(
                    writer, sheet_name="RMSERA-MCRBANDS"[:31], index=False
                )
