        ...
        if "mcr_S" in st.session_state:
            ...
            if "mcr_diag" in st.session_state:
                            ...
                            if decisions_id:
                                ...
                                if not selected_refs_rob:
                                    ...
                                    if st.button(t("🔁 Jalankan Uji Robustness", "🔁 Run Robustness Test"),
                                        ...
                                        S_ref_arr_rob = np.array(selected_refs_rob)
                                        # ── Cache: lewati komputasi ulang kalau data/konfigurasi
                                        # persis sama dengan run terakhir (klik ulang tombol yang
                                        # sama tanpa perubahan apa pun tidak menghitung ulang dari nol)
                                        import hashlib
                                        cache_key_rob = hashlib.sha256(
                                            D.tobytes() + S_ref_arr_rob.tobytes() +
                                            ...
                                            with st.spinner(t("Menjalankan grid + bootstrap robustness...",
                                                ...
                                                try:
                                                    ...
                                                    if fast_mode_rob:
                                                        ...
                                                        grid_kwargs["preprocessing_variants"] = ("raw", "snv")
                                        ...
                                        if "robustness_real_verdict" in st.session_state:
                                    ...
                                    if "robustness_real_verdict" in st.session_state:
                                        ...
                                        for d in st.session_state["identity_decisions"]:
                ...
                with st.expander(
                    ...
                    st.dataframe(existence_evidence_dfs["contribution"], use_container_width=True, hide_index=True)
                    if not existence_evidence_dfs["redundancy"].empty:
                        ...
                        st.dataframe(existence_evidence_dfs["redundancy"], use_container_width=True, hide_index=True)
                    ...
                    with st.expander(f"{t('Komponen','Component')} {i+1}", expanded=(i==0)):
                        ...
                        if results and len(results) >= 2 and results[0].get("ambiguous"):
                            ...
                            if labels_tb and names_tb == labels_tb:
                                amb_all = st.session_state.get("mcr_ambiguity")
                                ...
                                if region_df_tb is not None and region_info_tb is not None:
                                    with st.expander(t(
                                        ...
                                        st.dataframe(region_df_tb, use_container_width=True, hide_index=True)
