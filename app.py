...
def _resolve_diagnostic_ok(claimed_name):
    """
    Cari apakah nama kandidat identitas (claimed_name, mis. "Jahe Merah
    Murni") sudah pernah diuji lewat "Uji Pemisahan Kelompok Replikat"
    (assess_group_separability(), tab Analisis MCR) dan diberi label yang
    cocok pada salah satu kelompok. Kalau cocok, diagnostic_ok diambil
    dari ada/tidaknya strong_regions -- bukti statistik bahwa kelompok
    ini terpisah dari kelompok pembandingnya melebihi variasi replikat
    alami. Kalau belum pernah diuji / tidak ada label yang cocok,
    kembalikan None (BUKAN False) -- makna aslinya adalah "belum
    dinilai", bukan "gagal", supaya verdict identitas jujur menyatakan
    bukti yang benar-benar hilang lewat flag "diagnostic_region_belum_
    dinilai", bukan diam-diam dianggap gagal.
    ...
    ok = bool(sep_result.get("strong_regions"))
    if ok:
        detail = t(
            f"🔬 Diagnostic (pemisahan replikat vs '{other_label}'): ✅ ditemukan "
            f"region pembeda kuat ({len(sep_result['strong_regions'])} region) — "
            f"melebihi variasi alami dalam kelompok.",
            f"🔬 Diagnostic (replicate separability vs '{other_label}'): ✅ strong "
            f"distinguishing region(s) found ({len(sep_result['strong_regions'])}) "
            f"— exceeds natural within-group variation."
        )
    else:
        detail = t(
            f"🔬 Diagnostic (pemisahan replikat vs '{other_label}'): ❌ tidak "
            f"ditemukan region pembeda yang cukup kuat dibanding variasi alami "
            f"dalam kelompok.",
            f"🔬 Diagnostic (replicate separability vs '{other_label}'): ❌ no "
            f"sufficiently strong distinguishing region found relative to "
            f"within-group variation."
        )
    return ok, detail
                    ...
                    if "mcr_ambiguity" not in st.session_state:
                        ...
                        if not library_entries_id:
                            ...
                            for i in range(nc):
                                ...
                                diagnostic_ok_i, diagnostic_detail_i = _resolve_diagnostic_ok(sel_ref[1])
