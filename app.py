...
with tab_mcr:
    if "spectra" not in st.session_state:
        ...
        normalize_S = b1.checkbox(t("Normalisasi S per iterasi","Normalize S per iteration"), value=False,
                        help=t("Normalisasi unit vector S setiap iterasi. Dinonaktifkan secara default — aktifkan hanya jika komponen skala sangat berbeda.",
                               "Unit-vector normalize S each iteration. Disabled by default — enable only if component scales differ greatly."))
        ...
        if st.button(f"▶  {t('Jalankan MCR-ALS','Run MCR-ALS')}",
            ...
            with st.spinner(t("Menjalankan MCR-ALS...","Running MCR-ALS...")):
                C, S, lof_hist, r2, conv, diag = run_mcr_als(
                    ...
                    closure, unimodal, normalize_S=normalize_S,
                    S_init=S_init_guess
                )
