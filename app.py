                            ...
                            if decisions_id:
                                ...
                                if not selected_refs_rob:
                                    ...
                                    n_reps_rob = st.slider(
                                        ...
                                        min_value=5, max_value=30, value=15, step=5, key="n_reps_robustness_real",
                                    )
                                    st.caption(t(
                                        f"Grid 3 init × 4 preprocessing = 12 kombinasi, plus "
                                        f"{n_reps_rob} replikat bootstrap → {12 + n_reps_rob} fit "
                                        f"MCR-ALS tambahan untuk {len(selected_labels_rob)} komponen "
                                        f"terpilih ({', '.join(selected_labels_rob)}). Bisa memakan "
                                        f"waktu beberapa menit.",
                                        f"3 init × 4 preprocessing grid = 12 combos, plus "
                                        f"{n_reps_rob} bootstrap reps → {12 + n_reps_rob} extra "
                                        f"MCR-ALS fits for {len(selected_labels_rob)} selected "
                                        f"component(s) ({', '.join(selected_labels_rob)}). May take "
                                        f"a few minutes."
                                    ))
