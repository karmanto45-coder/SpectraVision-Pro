...
from existence_engine import (compute_contribution_ratio,
                              detect_spectral_redundancy,
                              build_existence_evidence_dataframe,
                              run_null_delta_lof_test)
from reproducibility_engine import (run_repeated_mcr,
                                    compute_existence_reproducibility,
                                    build_reproducibility_dataframe)
from existence_identity_decision_tree import (classify_component_existence_identity,
                                              build_decision_tree_dataframe)
from cos2d import (compute_2dcos, find_crosspeaks, apply_nodas_rules,
                   PERTURBATION_PRESETS)


# ── diagnostic_ok wiring (Tier 2, evaluate_component_identity) ──────────
def _resolve_diagnostic_ok(claimed_name, S_i=None, wn=None, amb_comp=None):
    """
    Verifikasi klaim identitas SPESIFIK komponen ini terhadap hasil "Uji
    Pemisahan Kelompok Replikat" -- BUKAN sekadar mengecek apakah uji itu
    pernah dijalankan untuk pasangan label ini secara umum.

    PERBAIKAN PENTING: versi sebelumnya mengembalikan `ok =
    bool(sep_result.get("strong_regions"))` -- ini SALAH, karena nilainya
    sama persis apa pun klaim yang diuji (klaim "Jahe Merah" ATAU "Jahe
    Emprit" pada komponen yang SAMA akan sama-sama dapat diagnostic_ok=True
    selama strong_regions ada di antara kedua kelompok itu secara umum).
    Itu membuat diagnostic_ok jadi stempel karet yang tidak membedakan
    klaim benar dari klaim salah -- persis penyebab regresi "semua
    komponen malah teridentifikasi sebagai kandidat yang salah", karena
    kandidat manapun yang dicoba akan sama-sama lolos tanpa benar-benar
    diverifikasi terhadap bentuk S_i yang sesungguhnya.

    Sekarang memanggil `_build_region_comparison_table()` (mesin yang sama
    dengan tabel "Perbandingan di region pembeda") untuk benar-benar
    membandingkan S_i terhadap KEDUA kelompok, lalu memutuskan:
    - pct_own >= 70%  -> True  (klaim ini didukung region pembeda)
    - pct_own <= 30%  -> False (klaim ini DITOLAK -- lebih cocok ke
                                 kandidat lain; ini penting: False, bukan
                                 None, supaya evaluate_component_identity
                                 tahu ini gagal diverifikasi, bukan sekadar
                                 belum dinilai)
    - di antaranya     -> None (belum bisa diputuskan dengan jelas)

    Returns (diagnostic_ok: bool|None, detail_caption: str|None)
    """
    if not claimed_name:
        return None, None
    sep_result = st.session_state.get("replicate_sep_result")
    sep_labels = st.session_state.get("replicate_sep_claim_labels")
    if not sep_result or not sep_labels:
        return None, None
    label_a, label_b = sep_labels
    claimed_lower = claimed_name.strip().lower()
    if label_a and claimed_lower == label_a.strip().lower():
        other_label = label_b or "kelompok pembanding"
    elif label_b and claimed_lower == label_b.strip().lower():
        other_label = label_a or "kelompok pembanding"
    else:
        return None, None

    if S_i is None or wn is None:
        # Belum ada spektrum komponen untuk diverifikasi -- tidak bisa
        # memutuskan apa pun, tetap None (bukan True/False).
        return None, None

    _, _, info = _build_region_comparison_table(claimed_name, S_i, wn, amb_comp)
    if info is None:
        # assess_group_separability tidak punya region pembeda sama sekali
        # untuk pasangan ini -- tidak ada dasar untuk memutuskan.
        return None, None

    pct_own = info["pct_own"]
    if pct_own >= 70:
        ok = True
        detail = t(
            f"🔬 Diagnostic (verifikasi region pembeda vs '{other_label}'): ✅ "
            f"{pct_own:.0f}% region pembeda MENDUKUNG klaim '{claimed_name}'.",
            f"🔬 Diagnostic (distinguishing-region verification vs '{other_label}'): "
            f"✅ {pct_own:.0f}% of distinguishing regions SUPPORT the '{claimed_name}' claim."
        )
    elif pct_own <= 30:
        ok = False
        detail = t(
            f"🔬 Diagnostic (verifikasi region pembeda vs '{other_label}'): ❌ "
            f"DITOLAK -- hanya {pct_own:.0f}% region pembeda mendukung "
            f"'{claimed_name}', mayoritas ({100-pct_own:.0f}%) justru lebih "
            f"cocok ke '{other_label}'. Klaim ini kemungkinan besar SALAH.",
            f"🔬 Diagnostic (distinguishing-region verification vs '{other_label}'): "
            f"❌ REJECTED -- only {pct_own:.0f}% of distinguishing regions support "
            f"'{claimed_name}', while most ({100-pct_own:.0f}%) actually match "
            f"'{other_label}' instead. This claim is likely WRONG."
        )
    else:
        ok = None
        detail = t(
            f"🔬 Diagnostic (verifikasi region pembeda vs '{other_label}'): 🟡 "
            f"belum jelas ({pct_own:.0f}% vs {100-pct_own:.0f}%) -- kedua "
            f"kandidat masih terlalu mirip bahkan di region pembeda.",
            f"🔬 Diagnostic (distinguishing-region verification vs '{other_label}'): "
            f"🟡 inconclusive ({pct_own:.0f}% vs {100-pct_own:.0f}%) -- both "
            f"candidates remain too similar even in the distinguishing regions."
        )
    return ok, detail


def _build_region_comparison_table(claimed_name, S_i, wn, amb_comp=None, max_regions=10):
    """
    Bandingkan nilai spektrum komponen hasil MCR (S_i) terhadap rata-rata
    Kelompok A/B TEPAT di region pembeda yang ditemukan
    assess_group_separability() -- ini yang menjawab "spektra apa
    sebenarnya komponen ini" secara terarah (bukan kemiripan spektrum
    penuh yang bisa "diencerkan" oleh ribuan titik yang memang mirip).

    Returns (dataframe, verdict_text, info) atau (None, None, None) kalau
    belum ada hasil uji separability / nama tidak cocok kelompok manapun.
    `info` adalah dict {"own_label", "other_label", "pct_own"} -- dipakai
    logika pemutus dasi (tie-breaker) di tab Identifikasi supaya tidak
    perlu mem-parsing ulang teks verdict.
    """
    sep_result = st.session_state.get("replicate_sep_result")
    sep_labels = st.session_state.get("replicate_sep_claim_labels")
    sep_wn = st.session_state.get("replicate_sep_wn")
    if not sep_result or not sep_labels or not claimed_name:
        return None, None, None
    label_a, label_b = sep_labels
    claimed_lower = claimed_name.strip().lower()
    if label_a and claimed_lower == label_a.strip().lower():
        own_label, other_label = label_a, (label_b or "Kelompok lain")
        own_mean_raw, other_mean_raw = sep_result["mean_a"], sep_result["mean_b"]
    elif label_b and claimed_lower == label_b.strip().lower():
        own_label, other_label = label_b, (label_a or "Kelompok lain")
        own_mean_raw, other_mean_raw = sep_result["mean_b"], sep_result["mean_a"]
    else:
        return None, None, None

    # Samakan grid kalau file uji separability & file MCR beda upload
    wn = np.asarray(wn, dtype=float)
    if sep_wn is not None and (len(sep_wn) != len(wn) or not np.allclose(sep_wn, wn)):
        own_mean = interpolate_spectrum(sep_wn, own_mean_raw, wn)
        other_mean = interpolate_spectrum(sep_wn, other_mean_raw, wn)
    else:
        own_mean, other_mean = own_mean_raw, other_mean_raw

    regions = sep_result["strong_regions"] or sep_result["candidate_regions"]
    if not regions:
        return None, None, None

    # ── Samakan SKALA own_mean/other_mean ke skala S_i (least-squares
    # best-fit), PERSIS metode yang dipakai SIRI
    # (compute_identity_robustness_index di identity_decision_engine.py).
    # WAJIB: MCR-ALS punya ambiguitas skala intensitas bawaan (C dan S
    # bisa dikalikan faktor berapa pun asal salah satunya dibagi faktor
    # yang sama, hasil kali C×S tetap identik) -- S_i BUKAN otomatis di
    # skala absorbansi yang sama dengan spektrum referensi mentah dari
    # file upload terpisah. Tanpa ini, perbandingan "lebih dekat ke mana"
    # bisa didominasi selisih skala, bukan selisih bentuk spektrum --
    # persis yang terjadi kalau own_mean/other_mean masih di skala
    # absorbansi asli (~0.03-0.07) sementara S_i sudah dalam skala
    # ternormalisasi MCR yang jauh lebih besar (~0.4-0.9).
    def _lstsq_scale(ref, target):
        energy = float(np.dot(ref, ref))
        if energy < 1e-12:
            return 1.0
        scale = float(np.dot(ref, target) / energy)
        return scale if scale > 0 else 1.0

    scale_own = _lstsq_scale(own_mean, S_i)
    scale_other = _lstsq_scale(other_mean, S_i)
    own_mean_scaled = own_mean * scale_own
    other_mean_scaled = other_mean * scale_other

    low_mask = (amb_comp["local_ambiguity"]["low_ambiguity_mask"]
                if amb_comp is not None else None)

    rows = []
    votes_own, votes_other, total_weight = 0.0, 0.0, 0.0
    for reg in regions[:max_regions]:
        idx = np.where((wn >= reg["wn_start"]) & (wn <= reg["wn_end"]))[0]
        if len(idx) == 0:
            continue
        s_val = float(np.mean(S_i[idx]))
        own_val = float(np.mean(own_mean_scaled[idx]))
        other_val = float(np.mean(other_mean_scaled[idx]))
        closer = own_label if abs(s_val - own_val) <= abs(s_val - other_val) else other_label
        amb_status = None
        if low_mask is not None:
            amb_status = t("aman", "safe") if bool(np.mean(low_mask[idx]) >= 0.5) \
                         else t("⚠️ ambigu", "⚠️ ambiguous")
        rows.append({
            t("Region (cm⁻¹)", "Region (cm⁻¹)"): f"{reg['wn_start']:.0f}–{reg['wn_end']:.0f}",
            "SNR": round(reg["max_snr"], 2),
            f"S_i ({claimed_name or 'C'})": round(s_val, 4),
            f"Mean {own_label} (scaled)": round(own_val, 4),
            f"Mean {other_label} (scaled)": round(other_val, 4),
            t("Lebih dekat ke", "Closer to"): closer,
            t("Ambiguitas lokal (MCR)", "Local MCR ambiguity"): amb_status if amb_status else "-",
        })
        w = reg["max_snr"]
        if closer == own_label:
            votes_own += w
    ...
    if total_weight > 0:
        pct_own = 100.0 * votes_own / total_weight
        verdict = t(
            f"Dari {len(rows)} region pembeda (dibobot SNR): "
            f"**{pct_own:.0f}%** mendukung '{own_label}', "
            f"**{100-pct_own:.0f}%** justru lebih dekat ke '{other_label}'. "
            f"(faktor skala least-squares: {own_label}×{scale_own:.3g}, "
            f"{other_label}×{scale_other:.3g} — MCR punya ambiguitas skala "
            f"intensitas bawaan, jadi kedua rata-rata disamakan dulu ke "
            f"skala S_i sebelum dibandingkan.)",
            f"Across {len(rows)} distinguishing regions (SNR-weighted): "
            f"**{pct_own:.0f}%** support '{own_label}', "
            f"**{100-pct_own:.0f}%** are actually closer to '{other_label}'. "
            f"(least-squares scale factors: {own_label}×{scale_own:.3g}, "
            f"{other_label}×{scale_other:.3g} — MCR has an inherent "
            f"intensity-scale ambiguity, so both means are rescaled to "
            f"S_i's scale before comparing.)"
        )
        info = {"own_label": own_label, "other_label": other_label, "pct_own": pct_own}
    else:
        verdict, info = None, None
    return df, verdict, info


def _recompute_replicate_separability():
    """
    Hitung ulang Uji Pemisahan Kelompok Replikat dari nilai widget yang
    SUDAH tersimpan di session_state (key: sep_own_spectra/sep_own_names/
    sep_own_wn, mcr_group_a/mcr_group_b, mcr_group_a_label/mcr_group_b_label),
    lalu simpan hasilnya ke session_state (replicate_sep_result/_wn/_names/
    _claim_labels).

    ALASAN FUNGSI INI ADA (bug fix): tab_mcr (berisi expander "Keputusan
    Identitas Kimia") didefinisikan di file ini SEBELUM tab_match (berisi
    expander "Uji Pemisahan Kelompok Replikat"). Streamlit menjalankan
    seluruh script dari atas ke bawah pada SETIAP rerun, terlepas dari tab
    mana yang sedang aktif di layar. Akibatnya, kalau perhitungan uji
    pemisahan hanya dilakukan inline di dalam blok tab_match (seperti
    sebelumnya), maka pada rerun YANG SAMA saat user baru saja menjalankan
    uji ini, blok tab_mcr di atasnya sudah terlanjur selesai digambar
    memakai session_state["replicate_sep_result"] versi LAMA (kosong/None)
    -- membuat diagnostic_ok selalu gagal dievaluasi (flag
    "diagnostic_region_belum_dinilai") sampai ada SATU rerun tambahan.
    Dengan memanggil fungsi ini SEKALI DI AWAL SCRIPT (sebelum tab_mcr
    dirender), session_state selalu memuat hasil uji pemisahan yang
    terbaru berdasarkan nilai widget yang sudah ada sebelum rerun ini
    dimulai -- sehingga tab_mcr tidak lagi membaca data basi.
    """
    # PERBAIKAN (anti hasil basi): hasil lama SELALU dibuang dulu. Sebelumnya,
    # kalau input tidak lagi valid (kelompok <2 kolom, file diganti/dihapus,
    # uji dimatikan, atau perhitungan gagal) fungsi ini `return None` TANPA
    # menghapus replicate_sep_result -> hasil uji lama tetap dipakai sebagai
    # bukti diagnostic_ok seolah masih berlaku.
    for _k in ("replicate_sep_result", "replicate_sep_wn",
               "replicate_sep_names", "replicate_sep_claim_labels"):
        st.session_state.pop(_k, None)
    if not st.session_state.get("mcr_use_replicate_avg"):
        return None
    if st.session_state.get("sep_own_uploader") is None:
        # file uji dihapus user -> buang juga datanya
        for _k in ("sep_own_wn", "sep_own_names", "sep_own_spectra"):
            st.session_state.pop(_k, None)
        return None
    try:
        _up = st.session_state.get("sep_own_uploader")
        try:
            _up.seek(0)
        except Exception:
            pass
        _nm = _up.name.lower()
        if _nm.endswith((".xlsx", ".xls")):
            _df = pd.read_excel(_up)
        else:
            _df = pd.read_csv(_up, sep=None, engine="python", comment="#")
        st.session_state["sep_own_wn"] = _df[_df.columns[0]].values.astype(float)
        st.session_state["sep_own_names"] = list(_df.columns[1:].astype(str))
        st.session_state["sep_own_spectra"] = _df[_df.columns[1:]].values.astype(float)
        try:
            _up.seek(0)
        except Exception:
            pass
        ...
        for _k in ("sep_own_wn", "sep_own_names", "sep_own_spectra"):
            st.session_state.pop(_k, None)
        return None
    spec_names_all = st.session_state.get("sep_own_names")
    spectra_all = st.session_state.get("sep_own_spectra")
    wn_own = st.session_state.get("sep_own_wn")
    group_a_names = st.session_state.get("mcr_group_a", [])
    group_b_names = st.session_state.get("mcr_group_b", [])
    if len(group_a_names) < 2 or len(group_b_names) < 2:
        return None
    if (any(n not in spec_names_all for n in group_a_names)
            or any(n not in spec_names_all for n in group_b_names)):
        return None
    try:
        ...
        label_a = st.session_state.get("mcr_group_a_label", "") or ""
        label_b = st.session_state.get("mcr_group_b_label", "") or ""
        name_a_disp = label_a.strip() or "Kelompok A"
        name_b_disp = label_b.strip() or "Kelompok B"
        sep_result = assess_group_separability(
            group_a_spectra, group_b_spectra, wn_own,
            name_a=name_a_disp, name_b=name_b_disp
        )
        st.session_state["replicate_sep_result"] = sep_result
        st.session_state["replicate_sep_wn"] = wn_own
        st.session_state["replicate_sep_names"] = (name_a_disp, name_b_disp)
        st.session_state["replicate_sep_claim_labels"] = (label_a.strip(), label_b.strip())
        return sep_result
    except Exception:
        # Jangan sampai pra-pass ini menggagalkan seluruh app kalau ada
        # data yang belum lengkap/tidak konsisten -- tab_match di bawah
        # akan tetap menampilkan error aslinya secara normal kalau ada.
        return None
...
# ════════════════════════════════════════════════════════════════════════
# INTEGRITAS STATE HASIL ANALISIS  (anti "cache basi" / hasil tercampur)
# ════════════════════════════════════════════════════════════════════════
# Streamlit menjalankan ulang seluruh script di tiap interaksi, sedangkan
# hasil analisis disimpan di st.session_state dan BERTAHAN sampai ada kode
# yang menghapusnya. Tiga lapisan ketergantungan:
#   1) DATA        -> (wavenumber, spectra hasil pra-pemrosesan). Kalau data
#                     atau pra-pemrosesan berubah, SEMUA hasil turunan basi.
#   2) HASIL MCR   -> mcr_C/mcr_S/... Kalau MCR dijalankan ulang (k/constraint
#                     berbeda), semua hasil pasca-MCR basi.
#   3) PASCA-MCR   -> ambiguitas, keputusan identitas, eksistensi, matching...
# Versi lama hanya menghapus sebagian kunci (mis. `mcr_ambiguity`, hasil
# turunan/derivative, keputusan identitas TIDAK pernah dihapus), sehingga
# pita ambiguitas milik run lama bisa tampil/diekspor berdampingan dengan
# komponen run baru (IndexError bila jumlah komponen berbeda; lebih buruk:
# angka salah yang tampak valid bila jumlahnya sama).
_KEYS_MCR_RESULT = [
    "mcr_C", "mcr_S", "mcr_lof", "mcr_r2", "mcr_ncomp", "mcr_converged",
    "mcr_diag", "mcr_warnings", "mcr_run_sig", "mcr_data_sig",
]
_KEYS_POST_MCR = [
    "mcr_S_proc", "mcr_proc_log", "match_results", "match_thresholds",
    "mcr_ambiguity", "identity_decisions",
    "existence_repro_runs", "existence_null_test",
    "existence_null_test_extra_idx", "existence_final_decisions",
    "existence_final_sig",
    "manual_cmp_result", "manual_cmp_names", "manual_cmp_raw",
    "cos2d_result", "cos2d_perturb", "cos2d_unit", "cos2d_name",
]
_KEYS_DATA_ONLY = [
    "robustness_real_grid", "robustness_real_boot",
    "robustness_real_verdict", "robustness_real_cache_key",
    "deriv_spectra", "deriv_wavenumber", "deriv_order", "deriv_poly",
    "deriv_window", "deriv_data_sig",
    "deriv_mcr_C", "deriv_mcr_S", "deriv_mcr_lof", "deriv_mcr_diag",
    "deriv_match_results", "deriv_match_meta",
]
_KEYS_DATA = ["wavenumber", "spectra", "spec_names", "_uploaded_filename", "_data_sig"]
# Widget yang nilainya terikat pada indeks/jumlah komponen MCR tertentu.
_WIDGET_KEYS_POST_MCR = ["ambig_comp_select", "postmcr_comp_select",
                         "extra_comp_select"]
_WIDGET_PREFIXES_POST_MCR = ("identity_ref_select_",)
_WIDGET_KEYS_DERIV = ["deriv_match_comp"]
...
def _invalidate_analysis(level):
    """
    Hapus hasil turunan yang sudah tidak valid.
      level="mcr"  : MCR dijalankan ulang  -> hapus lapisan pasca-MCR
                     (hasil MCR lama akan ditimpa oleh pemanggil).
      level="data" : data/pra-pemrosesan berubah -> hapus hasil MCR + pasca-MCR
                     + semua hasil domain turunan + uji robustness data-riil.
      level="all"  : seperti "data" + hapus data itu sendiri (file dihapus /
                     gagal dibaca).
    """
    for k in _KEYS_POST_MCR:
        _safe_pop(k)
    for k in _WIDGET_KEYS_POST_MCR:
        _safe_pop(k)
    for k in list(st.session_state.keys()):
        if isinstance(k, str) and k.startswith(_WIDGET_PREFIXES_POST_MCR):
            _safe_pop(k)
    if level in ("data", "all"):
        for k in _KEYS_MCR_RESULT + _KEYS_DATA_ONLY + _WIDGET_KEYS_DERIV:
            _safe_pop(k)
    if level == "all":
        for k in _KEYS_DATA:
            _safe_pop(k)
...
def _get_valid_ambiguity():
    """
    Kembalikan hasil ambiguitas HANYA kalau memang milik hasil MCR yang sedang
    aktif (tanda tangan run + jumlah komponen + panjang grid + bentuk spektrum
    nominal cocok dengan mcr_S). Kalau tidak -> dibuang dan None dikembalikan,
    sehingga tidak pernah ada pita ambiguitas run lama yang ikut dipakai.
    """
    amb = st.session_state.get("mcr_ambiguity")
    if amb is None:
        return None
    ok = False
    try:
        S = st.session_state.get("mcr_S")
        comps = amb.get("components", [])
        ok = (
            S is not None
            and amb.get("_mcr_sig") is not None
            and amb.get("_mcr_sig") == st.session_state.get("mcr_run_sig")
            and len(comps) == int(st.session_state.get("mcr_ncomp", -1))
            and len(comps) == S.shape[0]
            and all(len(c["S_nominal"]) == S.shape[1]
                    and len(c["S_band_min"]) == S.shape[1]
                    and len(c["S_band_max"]) == S.shape[1] for c in comps)
        )
        if ok:
            # Verifikasi isi: bentuk profil nominal harus identik dengan S
            # run aktif (kosinus ~1; skala boleh beda -> dinormalisasi).
            for i, c in enumerate(comps):
                a = np.asarray(c["S_nominal"], dtype=float)
                b = np.asarray(S[i], dtype=float)
                na, nb = np.linalg.norm(a), np.linalg.norm(b)
                if na < 1e-12 or nb < 1e-12:
                    continue
                if float(a @ b / (na * nb)) < 0.9999:
                    ok = False
                    break
    except Exception:
        ok = False
    if not ok:
        _safe_pop("mcr_ambiguity")
        return None
    return amb


def _existence_evidence_sig():
    """Sidik jari bukti yang dipakai 'Gabungkan Semua Bukti' (keputusan akhir)."""
    idn = st.session_state.get("identity_decisions") or []
    idn_part = sorted((d.get("component_index"), d.get("final_label")) for d in idn)
    rep = st.session_state.get("existence_repro_runs")
    rep_part = (None if rep is None else
                [(r.get("component_index"), round(float(r.get("p_repro_pct") or 0), 6))
                 for r in rep])
    nul = st.session_state.get("existence_null_test")
    nul_part = None if nul is None else str(nul.get("classification", ""))
    return _hashlib.sha256(repr((
        st.session_state.get("mcr_run_sig"), idn_part, rep_part, nul_part,
        st.session_state.get("existence_null_test_extra_idx"),
    )).encode()).hexdigest()[:24]

# ── Pra-pass: refresh hasil Uji Pemisahan Kelompok Replikat SEBELUM
# tab_mcr (yang berisi "Keputusan Identitas Kimia") dirender di bawah.
# Lihat docstring _recompute_replicate_separability() untuk alasannya.
_recompute_replicate_separability()
...
with st.sidebar:
    ...
    lang = st.session_state["lang"]   # t() membaca global `lang` -> berlaku langsung

    st.markdown("---")
    if st.button(t("Keluar","Logout"), width="stretch"):
        logout()
        st.rerun()
...
with tab_input:
    ...
    if uploaded:
        try:
            ...
            # ── Validasi data (baris kosong/NaN tidak boleh masuk ke MCR) ──
            _ok_rows = np.isfinite(wavenumber) & np.all(np.isfinite(raw_matrix), axis=1)
            _n_bad = int((~_ok_rows).sum())
            if _n_bad:
                wavenumber = wavenumber[_ok_rows]
                raw_matrix = raw_matrix[_ok_rows]
                st.warning(t(
                    f"⚠️ {_n_bad} baris berisi sel kosong/non-numerik dibuang otomatis "
                    f"(sisa {len(wavenumber)} titik data).",
                    f"⚠️ {_n_bad} row(s) with empty/non-numeric cells were dropped "
                    f"automatically ({len(wavenumber)} data points remain)."))
            if len(spec_cols) < 3:
                raise ValueError(t(
                    f"Minimal 3 spektra (kolom sampel) diperlukan; file ini hanya "
                    f"memiliki {len(spec_cols)}.",
                    f"At least 3 spectra (sample columns) are required; this file "
                    f"has only {len(spec_cols)}."))
            if len(wavenumber) < 20:
                raise ValueError(t(
                    f"Titik data terlalu sedikit ({len(wavenumber)}) — minimal 20.",
                    f"Too few data points ({len(wavenumber)}) — at least 20 needed."))
            if np.any(np.diff(np.sort(wavenumber)) == 0):
                raise ValueError(t(
                    "Ada nilai wavenumber duplikat pada kolom pertama.",
                    "Duplicate wavenumber values found in the first column."))
            ...
            # Deteksi perubahan berbasis ISI data (bukan cuma nama+ukuran file):
            # file lain bernama/berukuran sama, atau mengubah opsi pra-pemrosesan
            # (normalisasi/smoothing/baseline), sama-sama membuat semua hasil
            # MCR & turunannya basi -> direset supaya tidak tercampur.
            _new_sig = _array_signature(wavenumber, proc)
            _old_sig = st.session_state.get("_data_sig")
            _had_results = any(_k in st.session_state for _k in
                               ("mcr_S", "deriv_spectra", "cos2d_result",
                                "match_results"))
            if _old_sig != _new_sig:
                _invalidate_analysis("data")
                if _had_results and _old_sig is not None:
                    st.warning(t(
                        "⚠️ Data atau pra-pemrosesan berubah — semua hasil analisis "
                        "sebelumnya (MCR, ambiguitas, identifikasi, turunan, 2D-COS) "
                        "direset agar tidak tercampur dengan data baru. Jalankan ulang.",
                        "⚠️ Data or preprocessing changed — all previous analysis "
                        "results (MCR, ambiguity, identification, derivative, 2D-COS) "
                        "were reset so they cannot mix with the new data. Please re-run."))
            ...
            st.session_state["_data_sig"]   = _new_sig
            ...
            # File gagal dibaca/tidak valid: JANGAN biarkan data & hasil dari
            # file sebelumnya tetap aktif (tab lain akan diam-diam memakainya
            # seolah file baru ini yang dianalisis).
            _invalidate_analysis("all")
            st.error(t(f"File tidak dapat dipakai — {e}",
                       f"File cannot be used — {e}"))

# ── Guard konsistensi state (pertahanan berlapis): kalau hasil MCR/turunan
# tersimpan tapi TIDAK cocok dengan data yang sedang aktif (tanda tangan data
# atau bentuk matriks beda), buang -- jangan pernah dipakai.
def _enforce_state_consistency():
    cur_sig = st.session_state.get("_data_sig")
    spectra = st.session_state.get("spectra")
    if "mcr_S" in st.session_state:
        bad = (spectra is None
               or st.session_state.get("mcr_data_sig") != cur_sig
               or st.session_state["mcr_C"].shape[0] != spectra.shape[1]
               or st.session_state["mcr_S"].shape[1] != spectra.shape[0]
               or st.session_state["mcr_S"].shape[0] != st.session_state.get("mcr_ncomp"))
        if bad:
            _invalidate_analysis("data")
    if "deriv_spectra" in st.session_state:
        bad = (spectra is None
               or st.session_state.get("deriv_data_sig") != cur_sig
               or st.session_state["deriv_spectra"].shape != (spectra.shape[1], spectra.shape[0]))
        if bad:
            for _k in _KEYS_DATA_ONLY:
                if _k.startswith("deriv_"):
                    _safe_pop(_k)
            _safe_pop("deriv_match_comp")

_enforce_state_consistency()
...
with tab_mcr:
    if "spectra" not in st.session_state:
        ...
        # k harus < jumlah sampel (k = n_sampel membuat model cocok sempurna
        # dengan data apa pun -> LOF ~0 tanpa makna); konsisten dengan batas
        # detect_components().
        _k_max = max(2, min(10, D.shape[0] - 1))
        n_comp   = a1.number_input(t("Jumlah komponen","Components"), 2, _k_max,
                                   min(auto_k, _k_max))
        ...
        # (Fitur "Uji pemisahan dua kelompok replikat" dipindahkan ke tab
        # Identifikasi -- fitur ini tidak memakai hasil MCR-ALS apa pun,
        # jadi lebih tepat berada di tempat hasilnya (diagnostic_ok) dipakai.)
            ...
            if not all_lib_sel:
                ...
                for fi in range(int(n_fixed)):
                    ...
                    comp_idx_sel = fc1.number_input(
                        t(f"Indeks komponen #{fi+1}",f"Component index #{fi+1}"),
        ...
        # ── Validasi opsi tambahan SEBELUM run: opsi yang dicentang tapi belum
        # valid dulu diabaikan diam-diam (MCR jalan tanpa constraint itu,
        # sementara user mengira constraint aktif). Sekarang run diblokir.
        _run_blockers = []
        if use_init_guess and S_init_guess is None:
            _run_blockers.append(t(
                "Initial guess library dicentang tetapi belum valid (pilih tepat "
                f"{int(n_comp)} spektra yang overlap dengan rentang data) — "
                "matikan opsinya atau lengkapi pilihan.",
                "Library initial guess is enabled but not valid (select exactly "
                f"{int(n_comp)} spectra overlapping the data range) — turn it "
                "off or complete the selection."))
        if use_selectivity and not fixed_spectra_dict:
            _run_blockers.append(t(
                "Selectivity constraint dicentang tetapi belum valid (library "
                "kosong / indeks komponen duplikat) — matikan atau perbaiki.",
                "Selectivity constraint is enabled but not valid (empty library / "
                "duplicate component indices) — turn it off or fix it."))
        if use_window and not fixed_conc_zero_list:
            _run_blockers.append(t(
                "Windowing constraint dicentang tetapi belum lengkap (pilih "
                "minimal 1 sampel DAN 1 komponen) — matikan atau lengkapi.",
                "Windowing constraint is enabled but incomplete (select at "
                "least 1 sample AND 1 component) — turn it off or complete it."))

        if st.button(f"▶  {t('Jalankan MCR-ALS','Run MCR-ALS')}",
                     width="stretch"):
            _mcr_ok = False
            if _run_blockers:
                for _b in _run_blockers:
                    st.error("⛔ " + _b)
            else:
                try:
                    with st.spinner(t("Menjalankan MCR-ALS...","Running MCR-ALS...")):
                        C, S, lof_hist, r2, conv, diag = run_mcr_als(
                            ...
                            S_init=S_init_guess if use_init_guess else None,
                    ...
                    # Run gagal: hasil lama (jika ada) tetap milik run lamanya --
                    # tidak ditimpa setengah-setengah.
                    st.error(t(f"MCR-ALS gagal dijalankan: {_e}",
                               f"MCR-ALS failed to run: {_e}"))

            if _mcr_ok:
                # Run BARU berhasil -> semua hasil pasca-MCR run lama basi: buang
                # SEBELUM menyimpan hasil baru (termasuk mcr_ambiguity, keputusan
                # identitas, eksistensi, dan pilihan widget per-komponen).
                _invalidate_analysis("mcr")
                st.session_state.update({
                    ...
                    "mcr_run_sig": _array_signature(C, S),
                    "mcr_data_sig": st.session_state.get("_data_sig"),
                })
            ...
            if "mcr_diag" in st.session_state:
                ...
                with st.expander(
                    ...
                    if amb is not None:
                        ...
                        fig_amb.add_trace(go.Scatter(
                            x=wn_axis, y=comp_data["S_nominal"], mode="lines",
                ...
                with st.expander(
                    ...
                    # Keputusan identitas SELALU dihitung ulang dari state aktif;
                    # buang versi lama supaya tidak ada keputusan basi tersisa
                    # kalau blok ini tidak menghasilkan keputusan baru.
                    st.session_state.pop("identity_decisions", None)
                    amb_id = _get_valid_ambiguity()
                    if amb_id is None:
                        st.info(t(
                        ...
                        if not library_entries_id:
                            ...
                            for i in range(nc):
                                ...
                                if lib_entry_id is None:
                                    st.warning(t(
                                        f"Reference ID {ref_id} tidak ditemukan di library (mungkin sudah dihapus) — komponen dilewati.",
                                        f"Reference ID {ref_id} not found in the library (possibly deleted) — component skipped."))
                                    continue
                                ...
                                if amb_comp_id is not None:
                                    # PERBAIKAN SKALA: S_band_min/max berasal dari
                                    # ambiguity_engine._normalize_rows() (unit-norm bila
                                    # closure=False), sedangkan S_res[i] adalah S mentah
                                    # run_mcr_als. SIRI menyamakan skala referensi ke
                                    # argumen S_nominal lalu membandingkannya dengan band;
                                    # kalau S_nominal berskala beda dari band, referensi
                                    # YANG BENAR pun jatuh di luar band (SIRI ~10% untuk
                                    # kecocokan kosinus 0.9999). Pakai S_nominal milik
                                    # engine ambiguitas -- satu-satunya pasangan yang
                                    # skalanya konsisten dengan band.
                                    siri_result_id = compute_identity_robustness_index(
                                        amb_comp_id["S_nominal"],
                                        amb_comp_id["S_band_min"], amb_comp_id["S_band_max"],
                                        S_ref_resampled,
                                    )

                                # ── Ambil diagnostic_ok dari hasil "Uji Pemisahan
                                # Kelompok Replikat" (assess_group_separability),
                                # KALAU nama kandidat identitas komponen ini cocok
                                # dengan salah satu kelompok yang sudah diberi label
                                # di tab Analisis MCR. Kalau belum pernah diuji /
                                # tidak ada yang cocok -> tetap None (jujur, sesuai
                                # makna asli parameter ini: "belum dinilai").
                                diagnostic_ok_i, diagnostic_detail_i = _resolve_diagnostic_ok(
                                    sel_ref[1], S_res[i], wn, amb_comp_id
                                )
                                ...
                                decision_id = evaluate_component_identity(
                                    ...
                                    diagnostic_ok=diagnostic_ok_i,
                                )
                                if diagnostic_detail_i:
                                    st.caption(diagnostic_detail_i)

                                # ── Tabel otomatis: S_i vs Mean Kelompok A/B
                                # tepat di region pembeda (jawaban langsung
                                # atas "spektra apa sebenarnya komponen ini",
                                # dibanding kemiripan spektrum penuh yang bisa
                                # diencerkan oleh titik-titik yang memang mirip)
                                region_df, region_verdict, _region_info = _build_region_comparison_table(
                                    sel_ref[1], S_res[i], wn, amb_comp_id
                                )
                                if region_df is not None:
                                    with st.expander(t(
                                        f"📊 Perbandingan di region pembeda vs '{sel_ref[1]}'",
                                        f"📊 Comparison at distinguishing regions vs '{sel_ref[1]}'"
                                    )):
                                        st.dataframe(region_df, width="stretch", hide_index=True)
                                        if region_verdict:
                                            st.markdown(region_verdict)
                                decision_id["component_index"] = i
                                decision_id["pearson"] = pear_id
                                decision_id["cosine"] = cos_id
                                decision_id["fit_ok"] = fit_ok_scorecard
                                decision_id["ambiguity_width_pct"] = amb_comp_id["ambiguity_width_pct"] if amb_comp_id else None
                                decision_id["ambiguity_reliability_label"] = amb_comp_id["reliability"]["label"] if amb_comp_id else None
                                decision_id["siri_result"] = siri_result_id
                                decision_id["diagnostic_ok"] = diagnostic_ok_i
                            ...
                            if decisions_id:
                                ...
                                st.caption(t(
                                    "Verdict 'reliably_identified'/'possible_false_resolution' di atas "
                                    "masih PROVISIONAL (robustness_ok=None) sampai Uji Robustness di "
                                    "bawah dijalankan untuk komponen yang bersangkutan.",
                                    "The 'reliably_identified'/'possible_false_resolution' verdicts "
                                    "above are still PROVISIONAL (robustness_ok=None) until the "
                                    "Robustness Test below is run for the component in question."
                                ))
                                st.session_state["identity_decisions"] = decisions_id

                                # ── Uji Robustness Identitas pada DATA REAL (mengisi robustness_ok) ──
                                st.markdown(f"**{t('🔁 Uji Robustness Identitas (Data Real)', '🔁 Identity Robustness Test (Real Data)')}**")
                                st.caption(t(
                                    "Menjalankan ULANG pipeline MCR-ALS lengkap di data mixture RIIL "
                                    "ini (bukan simulasi sintetis) di bawah variasi metode inisialisasi "
                                    "(PCA/SIMPLISMA/NMF), preprocessing (raw/baseline+norm/smooth+"
                                    "baseline+norm/SNV), dan noise bootstrap — lalu memeriksa apakah "
                                    "verdict identitas terhadap reference yang SUDAH Anda pilih di atas "
                                    "tetap konsisten. Ini yang mengisi robustness_ok, menggantikan "
                                    "placeholder None. Hanya menguji komponen yang sudah punya "
                                    "reference terpilih (bukan '— pilih reference —').",
                                    "RE-runs the full MCR-ALS pipeline on THIS REAL mixture data (not "
                                    "synthetic simulation) under variation of init method (PCA/"
                                    "SIMPLISMA/NMF), preprocessing (raw/baseline+norm/smooth+baseline+"
                                    "norm/SNV), and noise bootstrap — then checks whether the identity "
                                    "verdict against the reference you already selected above stays "
                                    "consistent. This is what fills in robustness_ok, replacing the "
                                    "None placeholder. Only tests components that already have a "
                                    "reference selected (not '— select reference —')."
                                ))

                                selected_refs_rob, selected_labels_rob = [], []
                                for i in range(nc):
                                    sel_key = f"identity_ref_select_{i}"
                                    sel_val = st.session_state.get(sel_key)
                                    if sel_val and sel_val[0] is not None:
                                        ref_entry_rob = get_spectrum_by_id(sel_val[0])
                                        if ref_entry_rob is None:
                                            continue
                                        S_ref_rob = interpolate_spectrum(
                                            ref_entry_rob["wavenumber"], ref_entry_rob["spectrum"], wn)
                                        selected_refs_rob.append(S_ref_rob)
                                        selected_labels_rob.append(f"C{i+1}")

                                if not selected_refs_rob:
                                    st.info(t(
                                        "Pilih minimal 1 reference identitas di atas dulu sebelum "
                                        "menjalankan uji robustness.",
                                        "Select at least 1 identity reference above before running "
                                        "the robustness test."
                                    ))
                                else:
                                    fast_mode_rob = st.checkbox(
                                        t("⚡ Mode hemat komputasi (disarankan di Streamlit Cloud)",
                                          "⚡ Compute-saving mode (recommended on Streamlit Cloud)"),
                                        value=True, key="robustness_fast_mode_real",
                                        help=t(
                                            "Grid 2 init × 2 preprocessing (4 kombinasi, bukan 12) + "
                                            "toleransi konvergensi lebih longgar khusus untuk fit "
                                            "sampingan ini (bukan fit MCR utama). Cukup untuk menguji "
                                            "konsistensi arah solusi, jauh lebih ringan untuk server "
                                            "gratis. Matikan untuk uji penuh 3×4 kalau dijalankan lokal "
                                            "atau di server dengan kuota lebih besar.",
                                            "2 init × 2 preprocessing grid (4 combos, not 12) + looser "
                                            "convergence tolerance for this side-fit only (not the main "
                                            "MCR fit). Enough to test solution-direction consistency, "
                                            "much lighter for free-tier hosting. Turn off for the full "
                                            "3×4 test when running locally or on a higher-quota server."
                                        )
                                    )
                                    n_reps_rob = st.slider(
                                        t("Jumlah replikat noise bootstrap", "Number of noise bootstrap reps"),
                                        min_value=4, max_value=30,
                                        value=6 if fast_mode_rob else 15, step=2,
                                        key="n_reps_robustness_real",
                                    )
                                    n_combos_rob = 4 if fast_mode_rob else 12
                                    # Kunci cache dihitung SEKARANG (bukan hanya saat tombol
                                    # ditekan) supaya hasil tersimpan yang berasal dari
                                    # data/reference/konfigurasi lain TIDAK ikut dipakai
                                    # (dan tidak disuntikkan ke keputusan identitas).
                                    _S_ref_arr_now = np.array(selected_refs_rob)
                                    cache_key_rob = _hashlib.sha256(
                                        D.tobytes() + np.asarray(wn, dtype=float).tobytes() +
                                        _S_ref_arr_now.tobytes() +
                                        str((nc, tuple(selected_labels_rob), n_reps_rob, fast_mode_rob)).encode()
                                    ).hexdigest()
                                    rob_is_current = (
                                        st.session_state.get("robustness_real_cache_key") == cache_key_rob
                                    )
                                    if (not rob_is_current
                                            and "robustness_real_verdict" in st.session_state):
                                        st.info(t(
                                            "Hasil uji robustness yang tersimpan berasal dari data/"
                                            "reference/konfigurasi yang berbeda dan TIDAK dipakai — "
                                            "jalankan ulang uji untuk pengaturan saat ini.",
                                            "The stored robustness result comes from different data/"
                                            "references/settings and is NOT used — re-run the test "
                                            "for the current settings."))
                                    st.caption(t(
                                        f"Grid {'2 init × 2 preprocessing' if fast_mode_rob else '3 init × 4 preprocessing'} "
                                        f"= {n_combos_rob} kombinasi, plus "
                                        f"{n_reps_rob} replikat bootstrap → {n_combos_rob + n_reps_rob} fit "
                                        f"MCR-ALS tambahan untuk {len(selected_labels_rob)} komponen "
                                        f"terpilih ({', '.join(selected_labels_rob)}).",
                                        f"{'2 init × 2 preprocessing' if fast_mode_rob else '3 init × 4 preprocessing'} "
                                        f"grid = {n_combos_rob} combos, plus "
                                        f"{n_reps_rob} bootstrap reps → {n_combos_rob + n_reps_rob} extra "
                                        f"MCR-ALS fits for {len(selected_labels_rob)} selected "
                                        f"component(s) ({', '.join(selected_labels_rob)})."
                                    ))
                                    if st.button(t("🔁 Jalankan Uji Robustness", "🔁 Run Robustness Test"),
                                                key="btn_robustness_real"):
                                        S_ref_arr_rob = _S_ref_arr_now
                                        # ── Cache: lewati komputasi ulang HANYA kalau data,
                                        # reference & konfigurasi persis sama dengan run terakhir
                                        if rob_is_current:
                                            st.info(t(
                                                "Data & konfigurasi sama seperti run terakhir — memakai "
                                                "hasil yang sudah ada (tidak dihitung ulang).",
                                                "Same data & config as the last run — reusing existing "
                                                "result (not recomputed)."
                                            ))
                                        else:
                                            # buang hasil lama dulu: kalau perhitungan baru gagal,
                                            # hasil run sebelumnya tidak boleh tetap tampil
                                            for _k in ("robustness_real_grid", "robustness_real_boot",
                                                       "robustness_real_verdict", "robustness_real_cache_key"):
                                                st.session_state.pop(_k, None)
                                            with st.spinner(t("Menjalankan grid + bootstrap robustness...",
                                                              "Running grid + bootstrap robustness...")):
                                                try:
                                                    sweep_mcr_kwargs = (
                                                        {"max_iter": 80, "tol": 1e-4} if fast_mode_rob else None
                                                    )
                                                    grid_kwargs = dict(
                                                        n_components=nc, mcr_kwargs=sweep_mcr_kwargs,
                                                    )
                                                    if fast_mode_rob:
                                                        grid_kwargs["init_methods"] = ("simplisma", "nmf")
                                                        # SNV dihindari di mode hemat: robustness_sweep_engine
                                                        # sendiri menyatakan SNV per-sampel merusak struktur bilinear
                                                        # D = C@S pada data campuran, sehingga varian ini cenderung
                                                        # membuat komponen yang benar tampak "tidak stabil".
                                                        # Mode penuh (3x4) tetap memuat SNV sebagai pembanding.
                                                        grid_kwargs["preprocessing_variants"] = ("raw", "baseline_norm")
                                                    grid_res_rob = run_robustness_grid(
                                                        D, wn, S_ref_arr_rob, selected_labels_rob,
                                                        **grid_kwargs,
                                                    )
                                                    boot_res_rob = run_noise_bootstrap_robustness(
                                                        D, wn, S_ref_arr_rob, selected_labels_rob,
                                                        n_components=nc, n_reps=n_reps_rob,
                                                        mcr_kwargs=sweep_mcr_kwargs,
                                                    )
                                                    verdict_rob = summarize_robustness(
                                                        grid_res_rob["summary"], boot_res_rob["summary"],
                                                        selected_labels_rob,
                                                    )
                                                    st.session_state["robustness_real_grid"] = grid_res_rob
                                                    st.session_state["robustness_real_boot"] = boot_res_rob
                                                    st.session_state["robustness_real_verdict"] = verdict_rob
                                                    st.session_state["robustness_real_cache_key"] = cache_key_rob
                                                    rob_is_current = True
                                                except Exception as e:
                                                    st.error(t(f"Uji robustness gagal: {e}",
                                                               f"Robustness test failed: {e}"))

                                    # PERBAIKAN: blok ini dulu berada DI DALAM `if st.button(...)` sehingga
                                    # robustness_ok hanya masuk ke keputusan identitas pada rerun klik tombol;
                                    # interaksi berikutnya (mis. 'Gabungkan Semua Bukti') memakai keputusan
                                    # tanpa robustness. Sekarang dijalankan di setiap rerun selama hasilnya valid.
                                    # ── Suntikkan robustness_ok ke keputusan identitas yang
                                    # sudah ada (menggantikan placeholder None) dengan
                                    # mengevaluasi ULANG evaluate_component_identity()
                                    # memakai bukti fit/similarity/ambiguity/SIRI yang SAMA
                                    # seperti sebelumnya, ditambah robustness_ok yang baru.
                                    # Dijalankan di LUAR percabangan cache-hit/cache-miss di
                                    # atas, supaya tetap terpakai walau hasilnya berasal dari
                                    # cache (bukan baru dihitung ulang).
                                    if (st.session_state.get("robustness_real_cache_key") == cache_key_rob
                                            and "robustness_real_verdict" in st.session_state):
                                        verdict_rob = st.session_state["robustness_real_verdict"]
                                        by_label = {d["component_label"]: d for d in decisions_id}
                                        for label, v in verdict_rob.items():
                                            old_d = by_label.get(label)
                                            if old_d is None:
                                                continue
                                            new_d = evaluate_component_identity(
                                                component_label=label,
                                                fit_ok=old_d["fit_ok"],
                                                pearson=old_d["pearson"], cosine=old_d["cosine"],
                                                ambiguity_width_pct=old_d["ambiguity_width_pct"],
                                                ambiguity_reliability_label=old_d["ambiguity_reliability_label"],
                                                siri_result=old_d["siri_result"],
                                                robustness_ok=v["robustness_ok"],
                                                # bawa diagnostic_ok dari first-pass -- JANGAN diam-diam
                                                # kembali ke None di sini, atau verdict yang tadinya
                                                # sudah punya bukti diagnostic akan kehilangannya lagi
                                                # setiap kali robustness sweep dijalankan ulang.
                                                diagnostic_ok=old_d.get("diagnostic_ok"),
                                            )
                                            new_d["component_index"] = old_d["component_index"]
                                            new_d["pearson"] = old_d["pearson"]
                                            new_d["cosine"] = old_d["cosine"]
                                            new_d["fit_ok"] = old_d["fit_ok"]
                                            new_d["ambiguity_width_pct"] = old_d["ambiguity_width_pct"]
                                            new_d["ambiguity_reliability_label"] = old_d["ambiguity_reliability_label"]
                                            new_d["siri_result"] = old_d["siri_result"]
                                            new_d["diagnostic_ok"] = old_d.get("diagnostic_ok")
                                            by_label[label] = new_d
                                        decisions_id_updated = list(by_label.values())
                                        st.session_state["identity_decisions"] = decisions_id_updated

                                    if (st.session_state.get("robustness_real_cache_key") == cache_key_rob
                                            and "robustness_real_verdict" in st.session_state):
                                        st.dataframe(
                                            build_robustness_dataframe(
                                                st.session_state["robustness_real_grid"],
                                                st.session_state["robustness_real_boot"],
                                                st.session_state["robustness_real_verdict"],
                                                selected_labels_rob,
                                            ),
                                            width="stretch", hide_index=True,
                                        )
                                        st.markdown(t("**Verdict identitas setelah robustness_ok terisi:**",
                                                      "**Identity verdict after robustness_ok is filled in:**"))
                                        for d in st.session_state.get("identity_decisions", []):
                                            if d["component_label"] not in selected_labels_rob:
                                                continue
                                            st.markdown(f"**{d['component_label']}** — "
                                                        f"{badge_map_id.get(d['final_label'], '-')}")
                                            st.caption(d["narrative"])

                # ── Eksistensi Komponen & Keputusan Akhir (Existence Framework) ──
                with st.expander(
                    t("🔬 Eksistensi Komponen & Keputusan Akhir (Existence Framework)",
                      "🔬 Component Existence & Final Decision (Existence Framework)"),
                    expanded=False
                ):
                    st.caption(t(
                        "Menjawab pertanyaan yang BERBEDA dari expander identitas di atas: "
                        "'apakah faktor MCR ini benar-benar komponen kimia independen (bukan "
                        "artefak over-factoring/noise)?' — bukan 'komponen kimia yang mana'. "
                        "Digabung lewat pohon keputusan 5-outcome: VALID / REAL_UNASSIGNED / "
                        "NON_IDENTIFIABLE / UNCERTAIN / SPURIOUS.",
                        "Answers a DIFFERENT question from the identity expander above: "
                        "'is this MCR factor really an independent chemical component (not "
                        "an over-factoring/noise artifact)?' — not 'which chemical component'. "
                        "Combined via a 5-outcome decision tree: VALID / REAL_UNASSIGNED / "
                        "NON_IDENTIFIABLE / UNCERTAIN / SPURIOUS."
                    ))

                    D_exist = st.session_state["spectra"].T  # (n_sampel x n_wavenumber), data ASLI

                    # 1) Contribution Ratio — murah, langsung dihitung
                    contrib_results = compute_contribution_ratio(C_res, S_res, D_exist)
                    redund_results = detect_spectral_redundancy(C_res, S_res)
                    st.markdown(f"**{t('1. Contribution Ratio & Redundansi Spektral', '1. Contribution Ratio & Spectral Redundancy')}**")
                    existence_evidence_dfs = build_existence_evidence_dataframe(contrib_results, redund_results)
                    st.dataframe(existence_evidence_dfs["contribution"], width="stretch", hide_index=True)
                    if not existence_evidence_dfs["redundancy"].empty:
                        st.caption(t("Pasangan komponen dengan indikasi redundansi/splitting:",
                                     "Component pairs with redundancy/splitting indication:"))
                        st.dataframe(existence_evidence_dfs["redundancy"], width="stretch", hide_index=True)

                    st.markdown(f"**{t('2. Reproducibility (multi-restart MCR-ALS)', '2. Reproducibility (multi-restart MCR-ALS)')}**")
                    n_runs_exist = st.slider(
                        t("Jumlah run (init/noise acak)", "Number of runs (random init/noise)"),
                        min_value=10, max_value=50, value=20, step=5, key="n_runs_existence",
                    )
                    if st.button(t("🔁 Jalankan Reproducibility", "🔁 Run Reproducibility"), key="btn_repro_exist"):
                        with st.spinner(t(f"Menjalankan MCR-ALS {n_runs_exist}x...", f"Running MCR-ALS {n_runs_exist}x...")):
                            try:
                                runs_exist = run_repeated_mcr(D_exist, nc, n_runs=n_runs_exist)
                                st.session_state["existence_repro_runs"] = compute_existence_reproducibility(
                                    runs_exist, canonical_S=S_res)
                            except Exception as e:
                                st.error(t(f"Reproducibility gagal: {e}", f"Reproducibility failed: {e}"))
                    if "existence_repro_runs" in st.session_state:
                        st.dataframe(build_reproducibility_dataframe(st.session_state["existence_repro_runs"]),
                                    width="stretch", hide_index=True)

                    st.markdown(f"**{t('3. Uji Null ΔLOF (opsional, cukup mahal)', '3. Null ΔLOF Test (optional, fairly expensive)')}**")
                    if nc < 2:
                        st.info(t("Butuh minimal 2 komponen untuk uji ini.", "Needs at least 2 components for this test."))
                    else:
                        default_extra = int(np.argmin([r["contribution_pct"] for r in contrib_results]))
                        extra_idx = st.selectbox(
                            t("Komponen yang diuji sebagai 'komponen tambahan' (M_{k-1} vs M_k)",
                              "Component tested as the 'extra' component (M_{k-1} vs M_k)"),
                            options=list(range(nc)), index=default_extra,
                            format_func=lambda i: f"C{i+1}", key="extra_comp_select",
                        )
                        n_null_exist = st.slider(
                            t("Jumlah dataset null (lebih besar = lebih presisi, lebih lambat)",
                              "Number of null datasets (more = more precise, slower)"),
                            min_value=10, max_value=100, value=30, step=10, key="n_null_existence",
                        )
                        st.caption(t(
                            f"Membandingkan model {nc-1} vs {nc} komponen ({2*n_null_exist} fit MCR-ALS "
                            f"tambahan) — bisa memakan waktu.",
                            f"Compares the {nc-1}- vs {nc}-component model ({2*n_null_exist} extra "
                            f"MCR-ALS fits) — this can take a while."
                        ))
                        if st.button(t("🎲 Jalankan Uji Null ΔLOF", "🎲 Run Null ΔLOF Test"), key="btn_null_exist"):
                            with st.spinner(t("Menjalankan null test...", "Running null test...")):
                                try:
                                    st.session_state["existence_null_test"] = run_null_delta_lof_test(
                                        D_exist, nc - 1, nc, n_null_datasets=n_null_exist)
                                    st.session_state["existence_null_test_extra_idx"] = extra_idx
                                except Exception as e:
                                    st.error(t(f"Uji null gagal: {e}", f"Null test failed: {e}"))
                        if "existence_null_test" in st.session_state:
                            nt = st.session_state["existence_null_test"]
                            st.write(nt.get("classification", nt))

                    st.markdown(f"**{t('4. Keputusan Akhir (pohon 5-outcome)', '4. Final Decision (5-outcome tree)')}**")
                    if st.button(t("🧭 Gabungkan Semua Bukti", "🧭 Combine All Evidence"), key="btn_final_decision"):
                        identity_by_idx = {d["component_index"]: d for d in st.session_state.get("identity_decisions", [])}
                        repro_by_idx = {r["component_index"]: r for r in st.session_state.get("existence_repro_runs", [])}
                        null_result = st.session_state.get("existence_null_test")
                        null_extra_idx = st.session_state.get("existence_null_test_extra_idx")

                        final_results = []
                        for i in range(nc):
                            contrib_i = contrib_results[i]
                            repro_i = repro_by_idx.get(i)
                            ident_i = identity_by_idx.get(i)
                            null_i = null_result if (null_result is not None and i == null_extra_idx) else None
                            final_results.append(classify_component_existence_identity(
                                component_label=f"C{i+1}",
                                p_repro_pct=repro_i["p_repro_pct"] if repro_i else None,
                                null_test_result=null_i,
                                contribution_pct=contrib_i["contribution_pct"],
                                pearson=ident_i["pearson"] if ident_i else None,
                                cosine=ident_i["cosine"] if ident_i else None,
                                identity_decision=ident_i,
                            ))
                        st.session_state["existence_final_decisions"] = final_results
                        st.session_state["existence_final_sig"] = _existence_evidence_sig()

                    if ("existence_final_decisions" in st.session_state
                            and st.session_state.get("existence_final_sig") != _existence_evidence_sig()):
                        st.info(t(
                            "Bukti berubah sejak '🧭 Gabungkan Semua Bukti' terakhir dijalankan "
                            "(keputusan identitas/reproducibility/null test berbeda) — keputusan "
                            "akhir lama disembunyikan. Klik ulang tombol itu.",
                            "Evidence changed since '🧭 Combine All Evidence' was last run "
                            "(identity/reproducibility/null-test differ) — the old final decision "
                            "is hidden. Click that button again."))
                    elif "existence_final_decisions" in st.session_state:
                        final_badge_map = {
                            "VALID": "🟢 VALID",
                            "REAL_UNASSIGNED": "🔵 " + t("Nyata, belum teridentifikasi", "Real, unassigned"),
                            "NON_IDENTIFIABLE": "🟡 NON-IDENTIFIABLE",
                            "UNCERTAIN": "🟠 UNCERTAIN",
                            "SPURIOUS": "🔴 SPURIOUS",
                            "EVIDENCE_INCOMPLETE": "⚫ " + t("Bukti belum lengkap", "Evidence incomplete"),
                        }
                        for res in st.session_state["existence_final_decisions"]:
                            st.markdown(f"**{res['component_label']}** — {final_badge_map.get(res['final_label'], res['final_label'])}")
                            st.caption(res.get("narrative", ""))
                        st.dataframe(build_decision_tree_dataframe(st.session_state["existence_final_decisions"]),
                                    width="stretch", hide_index=True)
                        st.caption(t(
                            "Catatan: robustness_ok kini terisi kalau Anda sudah menjalankan '🔁 Uji "
                            "Robustness Identitas (Data Real)' di expander 'Keputusan Identitas Kimia' "
                            "di atas SEBELUM menekan tombol 'Gabungkan Semua Bukti' — verdict VALID "
                            "di sini akan otomatis ikut memakai robustness_ok terbaru. Kalau belum "
                            "dijalankan, robustness_ok masih None dan verdict tetap bersifat provisional.",
                            "Note: robustness_ok is now filled in if you already ran '🔁 Identity "
                            "Robustness Test (Real Data)' in the 'Chemical Identity Decision' expander "
                            "above BEFORE clicking 'Combine All Evidence' — VALID verdicts here will "
                            "automatically pick up the latest robustness_ok. If not run yet, "
                            "robustness_ok is still None and the verdict stays provisional."
                        ))
...
with tab_postmcr:
    if "mcr_S" not in st.session_state:
        ...
        if col_btn1.button(
            ...
            elif do_sg and not ((int(sg_window) % 2 == 1) and (int(sg_window) > int(sg_poly) + 2)
                                and int(sg_window) <= S.shape[1]):
                st.error(t(
                    f"⛔ Parameter smoothing tidak valid (window={int(sg_window)}, "
                    f"poly={int(sg_poly)}, titik data={S.shape[1]}) — perbaiki dulu.",
                    f"⛔ Invalid smoothing parameters (window={int(sg_window)}, "
                    f"poly={int(sg_poly)}, data points={S.shape[1]}) — fix them first."))
            else:
...
# ════════════════════════════════════════════════════════════════
with tab_match:
    # ── Uji Pemisahan Kelompok Replikat: SELALU tersedia di sini,
    # tidak bergantung pada MCR-ALS sudah dijalankan atau belum,
    # dan tidak bergantung pada Library terisi atau tidak -- fitur
    # ini pra-analisis mandiri, bisa dikerjakan kapan saja.
    # ── Uji Pemisahan Kelompok Replikat (opsional) ──────────────
    # Dipindahkan ke sini (dari tab Analisis MCR) karena fitur ini
    # TIDAK memakai hasil MCR-ALS (C/S) sama sekali -- ia hanya
    # butuh kolom mentah dari tab Input Data. Hasilnya (diagnostic_ok)
    # dipakai persis di tab ini, jadi lebih masuk akal ditempatkan
    # berdampingan dengan konsumennya, bukan di tab konfigurasi MCR.
    with st.expander(t(
        "🔬 Uji Pemisahan Kelompok Replikat (opsional — bukti diagnostic_ok)",
        "🔬 Replicate Group Separability Test (optional — diagnostic_ok evidence)"
    )):
        use_multi_replicate = st.checkbox(
            t("Uji pemisahan dua kelompok replikat",
              "Test separability between two replicate groups"),
            value=False, key="mcr_use_replicate_avg",
            help=t(
                "BUKAN menggabungkan Kelompok A dan Kelompok B menjadi satu "
                "referensi. Kelompok A dirata-rata SENDIRI (mis. beberapa "
                "replikat jahe merah), Kelompok B dirata-rata SENDIRI (mis. "
                "beberapa replikat jahe emprit), lalu kedua rata-rata itu "
                "dibandingkan untuk menguji apakah perbedaannya melebihi "
                "variasi alami di dalam masing-masing kelompok.",
                "This does NOT merge Group A and Group B into one reference. "
                "Group A is averaged SEPARATELY (e.g. several red-ginger "
                "replicates), Group B is averaged SEPARATELY (e.g. several "
                "white-ginger replicates), then the two averages are compared "
                "to test whether their difference exceeds natural within-group "
                "variation."
            )
        )
        if use_multi_replicate:
            from mcr_replicate_extension import assess_group_separability, build_separability_figure

            st.info(t(
                "ℹ️ Kelompok A dan Kelompok B TIDAK digabung menjadi satu "
                "referensi. Masing-masing dirata-rata sendiri, lalu keduanya "
                "dibandingkan untuk menguji apakah berbeda secara statistik "
                "(melebihi variasi replikat alami) — hasilnya dipakai sebagai "
                "bukti tambahan (diagnostic_ok) di bawah, bukan sebagai "
                "initial guess MCR.",
                "ℹ️ Group A and Group B are NOT merged into a single reference. "
                "Each is averaged separately, then the two are compared to test "
                "whether they differ statistically (beyond natural replicate "
                "variation) — the result is used as supporting evidence "
                "(diagnostic_ok) below, not as an MCR initial guess."
            ))

            # ── Data uploader MANDIRI, TERPISAH dari tab Input Data ──
            # Sengaja tidak memakai st.session_state["spectra"] (data
            # sampel campuran yang dipakai MCR-ALS). Ini pra-analisis
            # berdiri sendiri memakai spektra MURNI referensi (mis.
            # beberapa replikat jahe merah & jahe emprit) yang boleh
            # datang dari file/sesi yang SAMA SEKALI BEDA dengan file
            # sampel campuran yang sedang/akan dianalisis MCR-ALS.
            st.caption(t(
                "📂 Upload file KHUSUS untuk uji ini (independen dari file "
                "sampel campuran di tab Input Data). Format sama: Kolom 1 = "
                "wavenumber, Kolom 2+ = spektra replikat (mis. beberapa "
                "pengukuran jahe merah murni + beberapa pengukuran jahe "
                "emprit murni dalam satu file).",
                "📂 Upload a DEDICATED file for this test (independent from "
                "the mixture sample file in the Input Data tab). Same format: "
                "Column 1 = wavenumber, Column 2+ = replicate spectra (e.g. "
                "several pure red-ginger + several pure white-ginger "
                "measurements in one file)."
            ))
            sep_uploaded = st.file_uploader(
                t("Upload file replikat murni (Excel/CSV)",
                  "Upload pure replicate file (Excel/CSV)"),
                type=["xlsx", "xls", "csv"], key="sep_own_uploader"
            )
            if sep_uploaded is not None:
                try:
                    _sname = sep_uploaded.name.lower()
                    if _sname.endswith((".xlsx", ".xls")):
                        _sdf = pd.read_excel(sep_uploaded)
                    else:
                        _sdf = pd.read_csv(sep_uploaded, sep=None, engine="python", comment="#")
                    sep_wn_own = _sdf[_sdf.columns[0]].values.astype(float)
                    sep_names_own = list(_sdf.columns[1:].astype(str))
                    sep_spectra_own = _sdf[_sdf.columns[1:]].values.astype(float)  # (n_wn x n_sampel)
                    st.session_state["sep_own_wn"] = sep_wn_own
                    st.session_state["sep_own_names"] = sep_names_own
                    st.session_state["sep_own_spectra"] = sep_spectra_own
                    st.success(t(
                        f"✅ {len(sep_names_own)} spektra dimuat ({sep_wn_own.min():.0f}"
                        f"–{sep_wn_own.max():.0f} cm⁻¹) — terpisah dari data Input Data.",
                        f"✅ {len(sep_names_own)} spectra loaded ({sep_wn_own.min():.0f}"
                        f"–{sep_wn_own.max():.0f} cm⁻¹) — separate from the Input Data upload."
                    ))
                except Exception as _e:
                    st.error(f"Error: {_e}")

            if "sep_own_spectra" not in st.session_state:
                st.info(t(
                    "Belum ada file diupload untuk uji ini.",
                    "No file uploaded for this test yet."
                ))
            else:
                spec_names_all = st.session_state["sep_own_names"]
                spectra_all = st.session_state["sep_own_spectra"]
                wn_own = st.session_state["sep_own_wn"]
                ...
                if len(group_a_names) >= 2 and len(group_b_names) >= 2:
                    ...
                    # ── Nama kandidat identitas DIMINTA DULU, SEBELUM grafik
                    # dibuat -- supaya legend grafik & tabel bisa langsung
                    # memakai nama sebenarnya (mis. "Red Ginger Powder"),
                    # bukan label generik "Kelompok A/B" yang tidak
                    # Dulu ini text_input bebas ketik, yang berarti user harus
                    # HAFAL/menyalin persis nama entry library (merepotkan
                    # kalau library-nya sudah banyak isinya). Sekarang diganti
                    # jadi selectbox yang menarik nama LANGSUNG dari library
                    # yang sama dipakai di dropdown "Reference identitas yang
                    # diklaim" pada Keputusan Identitas Kimia -- jadi user
                    # tinggal PILIH, dijamin selalu sama persis (tidak mungkin
                    # typo/beda kapitalisasi), tidak perlu buka tab lain dulu.
                    st.caption(t(
                        "Pilih nama kandidat identitas untuk tiap kelompok LANGSUNG "
                        "dari library (dijamin sama persis, tidak perlu mengetik) -- "
                        "dipakai sebagai label legend grafik di bawah, dan juga "
                        "otomatis menjadi bukti diagnostic_ok saat menilai identitas "
                        "komponen itu nanti. Boleh dikosongkan (tetap dianalisis, "
                        "hanya legendnya generik 'Kelompok A/B').",
                        "Pick the claimed identity name for each group DIRECTLY "
                        "from the library (guaranteed to match exactly, no typing "
                        "needed) -- used as the chart legend label below, and "
                        "automatically becomes diagnostic_ok evidence when "
                        "evaluating that component's identity later. May be left "
                        "blank (still analyzed, just with generic 'Group A/B' legend)."
                    ))
                    try:
                        from database import get_all_spectra_for_matching as _get_lib_names_fn
                        _lib_entries_sep = _get_lib_names_fn() or []
                    except Exception:
                        _lib_entries_sep = []
                    _lib_name_opts = [""] + sorted({
                        e.get("name", f"#{e['id']}") for e in _lib_entries_sep
                    })
                    _placeholder = t("— pilih dari library —", "— pick from library —")
                    lcol1, lcol2 = st.columns(2)
                    if not _lib_name_opts[1:]:
                        st.info(t(
                            "Belum ada spektra di library -- tambahkan dulu lewat "
                            "tab Kelola Library supaya bisa dipilih di sini.",
                            "No spectra in the library yet -- add some via the "
                            "Manage Library tab first so they can be picked here."
                        ))
                    label_a = lcol1.selectbox(
                        t("Nama kandidat identitas Kelompok A", "Group A claimed identity name"),
                        options=_lib_name_opts,
                        format_func=lambda x: x if x else _placeholder,
                        key="mcr_group_a_label",
                    )
                    label_b = lcol2.selectbox(
                        t("Nama kandidat identitas Kelompok B", "Group B claimed identity name"),
                        options=_lib_name_opts,
                        format_func=lambda x: x if x else _placeholder,
                        key="mcr_group_b_label",
                    )
                    name_a_disp = label_a.strip() or "Kelompok A"
                    name_b_disp = label_b.strip() or "Kelompok B"

                    # Dihitung lewat helper bersama (bukan inline lagi) supaya
                    # logikanya identik dengan pra-pass di awal script yang
                    # menyuplai tab_mcr -- lihat _recompute_replicate_separability().
                    sep_result = _recompute_replicate_separability()
                    if sep_result is None:
                        # Fallback langsung (bukan lewat helper) supaya kalau
                        # memang ada error data, traceback aslinya tetap
                        # tampil seperti perilaku semula -- bukan silently
                        # tertelan seperti di pra-pass awal script.
                        sep_result = assess_group_separability(
                            group_a_spectra, group_b_spectra, wn_own,
                            name_a=name_a_disp, name_b=name_b_disp
                        )
                    ...
                    st.markdown(sep_result["overall_verdict"])
                    fig_sep = build_separability_figure(sep_result, wn_own, name_a_disp, name_b_disp)
                    st.plotly_chart(fig_sep, width="stretch")

                    if sep_result["strong_regions"]:
                        st.markdown(t("**Region pembeda kuat ditemukan:**",
                                      "**Strong distinguishing regions found:**"))
                        for reg in sep_result["strong_regions"][:8]:
                            st.write(f"- {reg['wn_start']:.0f}–{reg['wn_end']:.0f} cm⁻¹ "
                                     f"(puncak SNR di {reg['peak_wn']:.0f} cm⁻¹, "
                                     f"SNR={reg['max_snr']:.2f})")
                    # ── FIX: sebelumnya "elif" -- kalau strong_regions ADA,
                    # candidate_regions (area hijau di grafik SNR) TIDAK
                    # PERNAH ditampilkan sama sekali, padahal grafiknya
                    # sendiri tetap menggambar area hijau itu. candidate_
                    # regions memakai ambang lebih longgar (SNR>=1.0) jadi
                    # SELALU mencakup/lebih lebar dari strong_regions
                    # (SNR>=3.0) -- keduanya independent, bukan exclusive,
                    # jadi harus ditampilkan sebagai "if" terpisah, dengan
                    # deduplikasi berbasis overlap rentang (bukan exact
                    # dict match, karena batas wn_start/wn_end kandidat
                    # biasanya lebih lebar dari versi "kuat"-nya).
                    non_overlapping_candidates = [
                        c for c in sep_result["candidate_regions"]
                        if not any(c["wn_start"] <= s["wn_end"] and c["wn_end"] >= s["wn_start"]
                                   for s in sep_result["strong_regions"])
                    ]
                    if non_overlapping_candidates:
                        st.markdown(t("**Region kandidat (belum kuat):**",
                                      "**Candidate regions (not yet strong):**"))
                        for reg in non_overlapping_candidates[:8]:
                            st.write(f"- {reg['wn_start']:.0f}–{reg['wn_end']:.0f} cm⁻¹ "
                                     f"(SNR={reg['max_snr']:.2f})")
                    if not sep_result["strong_regions"] and not non_overlapping_candidates:
                        st.info(t(
                            "Tidak ada region pembeda (kuat maupun kandidat) yang "
                            "ditemukan pada ambang saat ini.",
                            "No distinguishing regions (strong or candidate) found "
    ...
    if "mcr_S" not in st.session_state:
        ...
        if n_lib == 0:
            ...
            st.session_state["match_thresholds"] = {
                "thresh_cos": float(thresh_cos), "thresh_hqi": float(thresh_hqi),
                "thresh_pearson": float(thresh_pearson),
            }
            ...
            if st.button(t("🔍 Jalankan identifikasi","🔍 Run identification"),
                         width="stretch", disabled=not ok_deriv_id):
            ...
            if "match_results" in st.session_state:
                ...
                for i, results in enumerate(all_results):
                    with st.expander(f"{t('Komponen','Component')} {i+1}", expanded=(i==0)):
                        ...
                        # ── PEMUTUS DASI: kalau kandidat #1 ditandai "ambiguous"
                        # (margin ke #2 terlalu tipis) DAN kedua kandidat itu
                        # persis sepasang kelompok yang sudah diuji di "Uji
                        # Pemisahan Kelompok Replikat", pakai tabel region
                        # pembeda (yang FOKUS hanya di titik yang benar-benar
                        # membedakan, tidak diencerkan ribuan titik yang mirip)
                        # sebagai bukti tambahan untuk memutuskan.
                        if results and len(results) >= 2 and results[0].get("ambiguous"):
                            top_name, second_name = results[0]["name"], results[1]["name"]
                            sep_labels_tb = st.session_state.get("replicate_sep_claim_labels")
                            names_tb = {top_name.strip().lower(), second_name.strip().lower()}
                            labels_tb = ({l.strip().lower() for l in sep_labels_tb if l}
                                         if sep_labels_tb else set())
                            if labels_tb and names_tb == labels_tb:
                                amb_all = _get_valid_ambiguity()
                                amb_comp_tb = (amb_all["components"][i]
                                               if amb_all and i < len(amb_all["components"]) else None)
                                region_df_tb, region_verdict_tb, region_info_tb = (
                                    _build_region_comparison_table(top_name, S[i], wn, amb_comp_tb)
                                )
                                if region_df_tb is not None and region_info_tb is not None:
                                    with st.expander(t(
                                        f"⚖️ Pemutus Dasi: Uji Region Pembeda ({top_name} vs {second_name})",
                                        f"⚖️ Tie-breaker: Distinguishing Region Test ({top_name} vs {second_name})"
                                    ), expanded=True):
                                        st.caption(t(
                                            f"Kandidat #1 ('{top_name}') dan #2 ('{second_name}') nyaris "
                                            f"seri (Δ={results[0].get('margin_to_next', 0):.4f}) pada "
                                            f"similarity spektrum PENUH. Tabel ini fokus HANYA di region "
                                            f"yang sudah terbukti membedakan kedua kelompok itu (dari Uji "
                                            f"Pemisahan Kelompok Replikat), yang tidak diencerkan oleh "
                                            f"ribuan titik lain yang memang mirip di kedua kandidat.",
                                            f"Candidate #1 ('{top_name}') and #2 ('{second_name}') are "
                                            f"nearly tied (Δ={results[0].get('margin_to_next', 0):.4f}) on "
                                            f"FULL-spectrum similarity. This table focuses ONLY on regions "
                                            f"already proven to distinguish the two groups (from the "
                                            f"Replicate Group Separability Test), undiluted by the "
                                            f"thousands of other points where both candidates look alike."
                                        ))
                                        st.dataframe(region_df_tb, width="stretch", hide_index=True)
                                        st.markdown(region_verdict_tb)
                                        pct_own_tb = region_info_tb["pct_own"]
                                        if pct_own_tb >= 70:
                                            st.success(t(
                                                f"✅ Ranking #1 ('{top_name}') DIKONFIRMASI oleh region "
                                                f"pembeda ({pct_own_tb:.0f}% dukungan).",
                                                f"✅ Rank #1 ('{top_name}') CONFIRMED by distinguishing "
                                                f"regions ({pct_own_tb:.0f}% support)."
                                            ))
                                        elif pct_own_tb <= 30:
                                            st.warning(t(
                                                f"🔄 Region pembeda justru mendukung '{second_name}' "
                                                f"({100-pct_own_tb:.0f}%), BUKAN '{top_name}' yang "
                                                f"peringkat #1 secara full-spectrum. Pertimbangkan "
                                                f"'{second_name}' sebagai identitas yang lebih tepat — "
                                                f"periksa manual sebelum memutuskan.",
                                                f"🔄 Distinguishing regions actually support '{second_name}' "
                                                f"({100-pct_own_tb:.0f}%), NOT the full-spectrum rank-#1 "
                                                f"'{top_name}'. Consider '{second_name}' as the more "
                                                f"likely identity — verify manually before deciding."
                                            ))
                                        else:
                                            st.info(t(
                                                f"Region pembeda juga belum bisa memutuskan dengan jelas "
                                                f"({pct_own_tb:.0f}% vs {100-pct_own_tb:.0f}%) — kedua "
                                                f"kandidat masih terlalu mirip bahkan di region yang "
                                                f"seharusnya membedakan.",
                                                f"Distinguishing regions also can't decide clearly "
                                                f"({pct_own_tb:.0f}% vs {100-pct_own_tb:.0f}%) — both "
                                                f"candidates remain too similar even in regions meant "
                                                f"to tell them apart."
                                            ))
                            else:
                                st.caption(t(
                                    f"💡 Kandidat #1/#2 nyaris seri. Kalau ingin pemutus dasi otomatis, "
                                    f"jalankan 'Uji Pemisahan Kelompok Replikat' dengan label persis "
                                    f"'{top_name}' dan '{second_name}'.",
                                    f"💡 Candidates #1/#2 are nearly tied. For an automatic tie-breaker, "
                                    f"run the 'Replicate Group Separability Test' with labels exactly "
                                    f"'{top_name}' and '{second_name}'."
                                ))
        ...
        if is_admin() and "mcr_S" in st.session_state:
            ...
            if res_manA and res_manB:
                ...
                if st.button(t("🔬 Bandingkan", "🔬 Compare"),
                             key="manual_cmp_btn", width="stretch",
...
with tab_cos:
    if "spectra" not in st.session_state:
        ...
        if st.button(t("▶ Jalankan 2D-COS","▶ Run 2D-COS"), width="stretch"):
        ...
        if "cos2d_result" in st.session_state:
            ...
            ex1.download_button(
                ...
                df_phi_exp.to_csv(), "synchronous_map.csv", "text/csv", width="stretch"
            )
            ex2.download_button(
                t("⬇ Asynchronous map (CSV)","⬇ Asynchronous map (CSV)"),
                df_psi_exp.to_csv(), "asynchronous_map.csv", "text/csv", width="stretch"
            )
...
with tab_deriv:
    ...
    if "spectra" not in st.session_state:
        ...
        if st.button(t("🧬 Buat Spektra Turunan","🧬 Generate Derivative Spectra"),
                    disabled=not ok_params, width="stretch"):
            # PERBAIKAN: turunan SG dihitung terhadap INDEKS titik. Pada file
            # wavenumber-menurun (4000->400) tandanya terbalik dibanding d/dv
            # yang sebenarnya (dan dibanding library yang dibuat dari sumbu naik).
            # Urutkan naik dulu sehingga tanda & orientasi selalu konsisten.
            _wn_d = np.asarray(wavenumber, dtype=float)
            _raw_d = np.asarray(raw_spectra, dtype=float)
            if _wn_d[0] > _wn_d[-1]:
                _wn_d, _raw_d = _wn_d[::-1], _raw_d[:, ::-1]
            try:
                deriv_matrix, deriv_info = apply_derivative(
                    _raw_d, order=deriv_order, poly=int(deriv_poly),
                    window=int(deriv_window), symmetric=True
                )
            except Exception as _e:
                st.error(t(f"Gagal membuat spektra turunan: {_e}",
                           f"Failed to generate derivative spectra: {_e}"))
                deriv_matrix = None
            if deriv_matrix is not None:
                st.session_state["deriv_spectra"]   = deriv_matrix
                st.session_state["deriv_wavenumber"] = _wn_d
                ...
                st.session_state["deriv_data_sig"]   = st.session_state.get("_data_sig")
                for _k in ["deriv_mcr_C","deriv_mcr_S","deriv_mcr_lof","deriv_mcr_diag",
                           "deriv_match_results","deriv_match_meta"]:
                    st.session_state.pop(_k, None)
                st.session_state.pop("deriv_match_comp", None)
        ...
        if "deriv_spectra" in st.session_state:
            ...
            _dnames = st.session_state.get("spec_names") or []
            if len(_dnames) != dsp.shape[0]:
                _dnames = [f"S{j+1}" for j in range(dsp.shape[0])]
            for i in range(n_show):
                fig_d.add_trace(go.Scatter(x=dwn, y=dsp[i], mode="lines", name=_dnames[i]))
            ...
            _kd_max = max(2, min(10, dsp.shape[0] - 1))
            n_comp_d = e1.number_input(t("Jumlah komponen (k)","Number of components (k)"),
                                       min_value=2, max_value=_kd_max, value=min(3, _kd_max),
                                       step=1, key="deriv_ncomp")
            # Opsi "library" dihapus: tidak pernah meneruskan S_init ke run_mcr_als,
            # jadi diam-diam berjalan sebagai PCA sambil tercatat "library".
            init_method_d = e2.selectbox(t("Metode inisialisasi","Init method"),
                                         ["pca"], index=0, key="deriv_init",
            ...
            if st.button(t("▶️ Jalankan MCR-ALS (Turunan)","▶️ Run MCR-ALS (Derivative)"), width="stretch"):
                try:
                    ...
                    # hasil matching lama milik komponen/run lama -> basi
                    st.session_state.pop("deriv_match_results", None)
                    st.session_state.pop("deriv_match_meta", None)
                    st.session_state.pop("deriv_match_comp", None)
                except Exception as _e:
                    st.error(t(f"MCR-ALS (turunan) gagal: {_e}",
                               f"MCR-ALS (derivative) failed: {_e}"))
            ...
            with st.expander(t("➕ Tambah acuan turunan ke library","➕ Add a derivative reference to the library")):
                ...
                if ref_upload and ref_name and "deriv_order" in st.session_state:
                    try:
                        ...
                        if r_wn[0] > r_wn[-1]:          # sumbu naik -> tanda turunan konsisten
                            r_wn, r_sp = r_wn[::-1], r_sp[::-1]
        ...
        if lib_meta_d:
            ...
            if picked:
                ...
                if all_wn_min:
                    ...
                    st.plotly_chart(fig_libprev, width="stretch")
        ...
        if "deriv_mcr_S" not in st.session_state:
            ...
            if st.button(t("🔎 Cocokkan dengan Library Turunan","🔎 Match against Derivative Library"), width="stretch"):
                ...
                if not lib_entries_d:
                    ...
                    st.session_state["deriv_match_meta"] = {
                        "comp_idx": int(comp_idx_d),
                        "order": st.session_state["deriv_order"],
                    }

            _dmeta = st.session_state.get("deriv_match_meta")
            if ("deriv_match_results" in st.session_state and _dmeta is not None
                    and _dmeta["order"] == st.session_state.get("deriv_order")
                    and _dmeta["comp_idx"] < S_d.shape[0]):
                st.markdown(t(
                    f"**Hasil matching untuk Komponen {_dmeta['comp_idx']+1}** "
                    f"(orde-{_dmeta['order']})",
                    f"**Matching result for Component {_dmeta['comp_idx']+1}** "
                    f"(order-{_dmeta['order']})"))
                ...
                if not ref_name:
                    ...
                    try:
                        ...
                        if ref_norm_method == "vector":
                            ...
                            from mcr_engine import abs_area as _abs_area
                            area = _abs_area(sp_r, wn_r)
    ...
    with tab_research:
        ...
        with sub_sim:
            ...
            if use_default_cases:
                st.dataframe(pd.DataFrame(DEFAULT_CASES), width="stretch", hide_index=True)
            ...
            if st.button(t("▶️ Jalankan Studi Simulasi", "▶️ Run Simulation Study"),
                        key="btn_run_sim_study", width="stretch"):
                if not sim_cases:
        ...
        with sub_rob:
            ...
            rob_fast_mode = st.checkbox(
                t("⚡ Mode hemat komputasi (konvergensi lebih longgar utk fit sampingan)",
                  "⚡ Compute-saving mode (looser convergence for side-fits)"),
                value=True, key="rob_sweep_fast_mode",
                help=t(
                    "max_iter & tol dilonggarkan khusus untuk fit di dalam sweep ini "
                    "(bukan fit MCR utama) -- lebih ringan untuk server gratis, cukup "
                    "untuk menguji konsistensi arah solusi.",
                    "max_iter & tol are loosened specifically for fits inside this "
                    "sweep (not the main MCR fit) -- lighter for free-tier hosting, "
                    "sufficient to test solution-direction consistency."
                )
            )

            if st.button(t("▶️ Jalankan Robustness Sweep", "▶️ Run Robustness Sweep"),
                        key="btn_run_rob_sweep", width="stretch"):
                if not rob_inits or not rob_preps:
                    ...
                    cache_key_sweep = hashlib.sha256(str((
                        rob_corr, rob_n_samples, rob_noise, tuple(sorted(rob_inits)),
                        tuple(sorted(rob_preps)), rob_n_boot, rob_fast_mode,
                    )).encode()).hexdigest()
                    if st.session_state.get("rob_sweep_cache_key") == cache_key_sweep:
                        st.info(t(
                            "Konfigurasi sama seperti run terakhir — memakai hasil yang "
                            "sudah ada (tidak dihitung ulang).",
                            "Same config as the last run — reusing existing result "
                            "(not recomputed)."
                        ))
                    else:
                        # buang hasil lama dulu: kalau run baru gagal, hasil konfigurasi
                        # sebelumnya tidak boleh tetap tampil di bawah pengaturan baru
                        for _k in ("rob_sweep_grid", "rob_sweep_boot", "rob_sweep_verdict",
                                   "rob_sweep_labels", "rob_sweep_cache_key"):
                            st.session_state.pop(_k, None)
                        with st.spinner(t(
                            ...
                            try:
                                ...
                                sweep_mcr_kwargs_2 = (
                                    {"max_iter": 80, "tol": 1e-4} if rob_fast_mode else None
                                )
                                ...
                                st.session_state["rob_sweep_cache_key"] = cache_key_sweep
                            except Exception as e:
        ...
        with sub_blind:
            ...
            if st.button(t("▶️ Jalankan Blind Validation", "▶️ Run Blind Validation"),
                        key="btn_run_blind", width="stretch"):
                if not blind_levels:
                    st.warning(t("Pilih minimal satu level korelasi.", "Select at least one correlation level."))
                else:
...
with tab_rep:
    if "mcr_S" not in st.session_state:
                    ...
                    for rank, r in enumerate(results, 1):
                        rows.append({
                            ...
                            t("Status","Status"): consensus_label(
                                ...
                                ambiguous=r.get("ambiguous", False),
                                **st.session_state.get("match_thresholds", {})
                            )[1]
                        })
        ...
        if "replicate_sep_result" in st.session_state:
            ...
            st.dataframe(df_sep_regions, width="stretch")
        ...
        # ── Laporan tambahan: Pita Ambiguitas Spektrum (nominal + feasible
        # band per komponen) -- dibaca dari st.session_state["mcr_ambiguity"]
        # (diisi di tab Identifikasi saat "Hitung Ambiguitas Rotasi" dijalankan).
        # Disusun sebagai Wavenumber vs Profil Nominal vs Band Min vs Band Max
        # per komponen (satu sheet per komponen) supaya bisa direproduksi
        # ulang sebagai grafik baru (mis. di Origin/GraphPad/matplotlib) untuk
        # keperluan publikasi jurnal, tanpa perlu screenshot dari aplikasi.
        # _get_valid_ambiguity(): hanya pita ambiguitas milik run MCR yang AKTIF
        # yang boleh diekspor (jumlah komponen, grid, dan bentuk profil
        # nominal diverifikasi terhadap mcr_S) -- tidak akan ada lagi
        # IndexError maupun pita run lama yang menempel pada komponen baru.
        amb_export = _get_valid_ambiguity()
        if amb_export is not None and "mcr_S" in st.session_state:
            st.markdown(f'<p class="sec-hdr">{t("Laporan Pita Ambiguitas Spektrum","Spectral Ambiguity Band Report")}</p>',
                        unsafe_allow_html=True)

            S_res_export = st.session_state["mcr_S"]
            wn_export = st.session_state["wavenumber"]
            nc_export = len(amb_export["components"])

            output_amb = io.BytesIO()
            with pd.ExcelWriter(output_amb, engine="openpyxl") as writer_amb:
                for i in range(nc_export):
                    comp_data_export = amb_export["components"][i]
                    df_amb = pd.DataFrame({
                        "Wavenumber (cm-1)": np.asarray(wn_export, dtype=float),
                        t(f"Profil_Nominal_C{i+1}", f"Nominal_Profile_C{i+1}"): comp_data_export["S_nominal"],
                        f"Band_Min_C{i+1}": comp_data_export["S_band_min"],
                        f"Band_Max_C{i+1}": comp_data_export["S_band_max"],
                    })
                    df_amb.to_excel(
                        writer_amb,
                        sheet_name=t(f"Komponen_{i+1}", f"Component_{i+1}")[:31],
                        index=False,
                    )
            output_amb.seek(0)

            st.caption(t(
                "Satu sheet per komponen: Wavenumber vs Profil Nominal (garis "
                "biru di grafik 'Pita Ambiguitas Spektrum') vs Band Min/Max "
                "(batas bawah/atas pita feasible) -- siap dipakai ulang untuk "
                "membuat grafik publikasi di software lain (Origin, GraphPad, "
                "matplotlib, dll.) tanpa perlu screenshot.",
                "One sheet per component: Wavenumber vs Nominal Profile (the "
                "blue line in the 'Spectral Ambiguity Band' chart) vs Band "
                "Min/Max (feasible band lower/upper bounds) -- ready to reuse "
                "for publication-quality plots in other software (Origin, "
                "GraphPad, matplotlib, etc.) without needing a screenshot."
            ))

            st.download_button(
                t("⬇ Download pita ambiguitas spektrum (Excel)",
                  "⬇ Download spectral ambiguity band (Excel)"),
                data=output_amb,
                file_name=f"Ambiguity_Band_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                width="stretch"
            )
