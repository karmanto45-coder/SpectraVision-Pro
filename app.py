...
def _build_region_comparison_table(claimed_name, S_i, wn, amb_comp=None, max_regions=10):
    ...
    Returns (dataframe, verdict_text) atau (None, None) kalau belum ada
    hasil uji separability / nama tidak cocok kelompok manapun.
    """
    sep_result = st.session_state.get("replicate_sep_result")
    sep_labels = st.session_state.get("replicate_sep_claim_labels")
    ...
    for reg in regions[:max_regions]:
        ...
        own_val = float(np.mean(own_mean[idx]))
        other_val = float(np.mean(other_mean[idx]))
        ...
        rows.append({
            ...
            f"Mean {own_label}": round(own_val, 4),
            f"Mean {other_label}": round(other_val, 4),
                    ...
                    if "mcr_ambiguity" not in st.session_state:
                        ...
                        if not library_entries_id:
                            ...
                            for i in range(nc):
                                ...
                                region_df, region_verdict = _build_region_comparison_table(
        ...
        if use_multi_replicate:
            ...
            if "sep_own_spectra" not in st.session_state:
                ...
                if len(group_a_names) >= 2 and len(group_b_names) >= 2:
                    ...
                    fig_sep = build_separability_figure(sep_result, wn_own, "Kelompok A", "Kelompok B")
                    ...
                    st.caption(t(
                        "Opsional: beri nama kandidat identitas untuk tiap kelompok "
                        "(harus SAMA PERSIS dengan nama di library, mis. 'Jahe Merah "
                        "Murni') supaya hasil pemisahan ini otomatis dipakai sebagai "
                        "bukti diagnostic_ok saat menilai identitas komponen itu di "
                        "bawah.",
                        "Optional: name the claimed identity for each group (must "
                        "EXACTLY match the library entry name) so this separability "
                        "result is automatically used as diagnostic_ok evidence when "
                        "evaluating that component's identity below."
                    ))
                    ...
                    elif sep_result["candidate_regions"]:
                        st.markdown(t("**Region kandidat (belum kuat):**",
                                      "**Candidate regions (not yet strong):**"))
                        for reg in sep_result["candidate_regions"][:8]:
                            st.write(f"- {reg['wn_start']:.0f}–{reg['wn_end']:.0f} cm⁻¹ "
                                     f"(SNR={reg['max_snr']:.2f})")
                else:
                    st.info(t(
