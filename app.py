    ...
    if "spectra" not in st.session_state:
        ...
        if use_multi_replicate:
            ...
            if len(group_a_names) >= 2 and len(group_b_names) >= 2:
                ...
                # ── Tag kelompok dengan nama kandidat identitas di library ──
                # Ini yang menghubungkan hasil separability ke diagnostic_ok
                # pada evaluate_component_identity() di tab Identifikasi.
                # Perhitungan assess_group_separability() sendiri murah
                # (operasi numpy O(n_wavenumber), bukan refit MCR-ALS
                # berulang seperti robustness sweep) -- jadi tidak ada
                # pertimbangan waktu komputasi untuk menyambungkannya di sini.
                st.caption(t(
                    ...
                    "evaluating that component's identity in the Identification tab."
                ))
