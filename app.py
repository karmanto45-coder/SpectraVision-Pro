        ...
        if use_multi_replicate:
            ...
            if sep_uploaded is not None:
                try:
                    ...
                    # ── Deteksi file BARU (bukan sekadar rerun dengan file yang
                    # sama) supaya nama klaim identitas Kelompok A/B tidak
                    # "nyangkut" dari analisis sebelumnya kalau Bapak ganti ke
                    # data referensi yang berbeda sama sekali. Tanpa ini, kotak
                    # nama (text_input) TIDAK ikut ter-reset otomatis seperti
                    # dropdown Kelompok A/B (yang memang divalidasi Streamlit
                    # terhadap opsi kolom baru), sehingga label lama bisa
                    # dipakai keliru untuk data baru tanpa disadari.
                    _sep_file_fingerprint = (sep_uploaded.name, sep_uploaded.size)
                    if st.session_state.get("sep_own_file_fingerprint") != _sep_file_fingerprint:
                        st.session_state["mcr_group_a_label"] = ""
                        st.session_state["mcr_group_b_label"] = ""
                        st.session_state["mcr_group_a"] = []
                        st.session_state["mcr_group_b"] = []
                        st.session_state["sep_own_file_fingerprint"] = _sep_file_fingerprint
            ...
            if "sep_own_spectra" not in st.session_state:
                ...
                if len(group_a_names) >= 2 and len(group_b_names) >= 2:
                    ...
                    # menjelaskan spektra apa yang sedang ditampilkan.
                    st.caption(t(
                        "Beri nama kandidat identitas untuk tiap kelompok (harus SAMA "
                        "PERSIS dengan nama di library, mis. 'Jahe Merah Murni') -- "
                        ...
                        "Name the claimed identity for each group (must EXACTLY match "
                        "the library entry name) -- used as the chart legend label "
                        "below, and automatically becomes diagnostic_ok evidence when "
                        "evaluating that component's identity later. May be left "
                        "blank (still analyzed, just with generic 'Group A/B' legend)."
                    ))
                    lcol1, lcol2 = st.columns(2)
                    label_a = lcol1.text_input(
                        t("Nama kandidat identitas Kelompok A", "Group A claimed identity name"),
                        key="mcr_group_a_label",
                    )
                    label_b = lcol2.text_input(
