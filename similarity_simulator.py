...
def run_single_replicate(wavenumber, target_correlation, noise_pct,
    ...
    for i, label in enumerate(labels_true):
        ...
        if amb_comp is not None:
            # PERBAIKAN SKALA SIRI: S_band_min/max berskala unit-norm (closure=False) dari
            # ambiguity_engine; argumen S_nominal WAJIB pasangan berskala sama (amb_comp["S_nominal"]),
            # bukan S mentah run_mcr_als, atau referensi yang benar pun jatuh di luar band.
            siri_result = compute_identity_robustness_index(
                amb_comp["S_nominal"], amb_comp["S_band_min"], amb_comp["S_band_max"], S_true[i],
            )
...
def run_correlation_noise_study(wavenumber, cases=None, n_replicates=10,
    ...
    for case in cases:
        ...
        for rep in range(n_replicates):
            # PERBAIKAN REPRODUCIBILITY: hash() bawaan Python untuk str/tuple diacak
            # PER PROSES (PYTHONHASHSEED), sehingga seed berubah tiap app/Streamlit
            # di-restart dan angka studi tidak bisa direproduksi. crc32 stabil lintas
            # proses & mesin.
            seed = base_seed + zlib.crc32(f"{case['case_name']}|{rep}".encode("utf-8")) % 1_000_000
