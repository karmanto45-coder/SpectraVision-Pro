...
def compute_rotational_ambiguity(C, S, constraints_used, method="auto",
    ...
    for comp in components:
        ...
        # Profil NOMINAL pada skala yang KONSISTEN dengan S_band_min/S_band_max
        # (keduanya berasal dari _normalize_rows() yang sama). PENTING: jangan
        # pakai S mentah dari run_mcr_als (mis. session_state["mcr_S"]) untuk
        # digambar/diekspor berdampingan dengan S_band_min/max -- kalau
        # closure=False, S mentah TIDAK senormalisasi dengan band (band di-
        # unit-norm-kan, S mentah tidak), menghasilkan mismatch skala drastis
        # (nominal bisa ribuan sementara band cuma 0.0x). Simpan di sini
        # supaya satu-satunya sumber kebenaran untuk pasangan nominal+band.
        comp["S_nominal"] = S_nom[i, :]
